...
```text
Build SAM Live Stock Action Engine v1. Every inbound Chatwoot message must produce a durable next_action from conversation state: answer_general_info, answer_location, answer_price, ask_one_missing_detail, prepare_draft_order, update_draft_order, prepare_quote, prepare_picture_response, no_reply_needed, or escalate. SAM must read Chatwoot history, intake/order state, stock availability, price rules, owner correction examples, and farm knowledge before drafting. It must prepare the work automatically but keep customer send, quote send, reservation, payment, and stock movement owner-gated. Acceptance: table-driven scenario tests for location, ad/business, pictures, price, Michaels-style collection timing, quote request, and natural close; no customer auto-send.
```
...
```text
Mission ID: CHARLIE-MISSION-6072D13D22A5CE75
Mission title: SAM Live Stock Action Engine v1
...
- technical_architect: pending - Design implementation structure, file/API/data impacts, integration risk, and test strategy.
