...
async function loadOwnerAttentionAll() {
  ...
  try {
    ...
    const renderItem = item => `<article role="listitem" class="attention-all-item"><span class="attention-emoji" aria-hidden="true">${attentionEscape(item.semantic_emoji)}</span><span><strong><a class="attention-detail-link" href="${attentionEscape(item.detail_target)}">${attentionEscape(item.title)}</a></strong><small>Status: ${attentionEscape(friendly(item.operational_status))} · Owner: ${attentionEscape(item.assigned_to)} · ${attentionEscape(friendly(item.freshness))}</small><em>${attentionEscape(item.exact_owner_action)}</em><details><summary>Details</summary><p>Reference: ${attentionEscape(item.secondary_reference)}</p><p>Source: ${attentionEscape(item.specialist_owner)}</p><p>Provenance: ${attentionEscape((item.provenance || []).join(" · "))}</p></details></span></article>`;
