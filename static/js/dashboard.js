...
async function loadFarm() {
  try {
    ...
    if (litters.length) {
      ...
      const sowName = first.sow_name || first.sow_tag_number || "";
      const headline = sowName || first.litter_id || "Litter";
      const litterContext = sowName && first.litter_id ? ` · ${first.litter_id}` : "";
