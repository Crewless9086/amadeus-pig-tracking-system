...
const lifecyclePreview = document.getElementById("lifecycle_preview");
const lifecycleConfirmButton = document.getElementById("lifecycle_confirm_button");
const mortalityAf = lifecycleForm?.dataset.language === "af";
let mortalityOperation = "";
...
function todayIsoDate(){return new Intl.DateTimeFormat('en-CA',{timeZone:'Africa/Johannesburg',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date())}
...
function renderIdentity(pig){
  ...
  const active=pig.status==="Active"&&pig.on_farm==="Yes";lifecyclePanel.classList.toggle("hidden",!active);if(lifecycleEventDate)lifecycleEventDate.max=todayIsoDate();
}
...
function setLifecycleSubmitting(busy){
  if(lifecycleSubmitButton)lifecycleSubmitButton.disabled=busy;
  if(lifecycleConfirmButton)lifecycleConfirmButton.disabled=busy;
}
function renderMortalityPreview(data){
  if(!lifecyclePreview)return;
  mortalityOperation=data.status==='preview_ready'?data.operation_id||'':'';
  lifecyclePreview.textContent=mortalityText(data.answer);
  lifecyclePreview.classList.toggle('hidden',!data.answer);
  lifecycleConfirmButton.classList.toggle('hidden',!mortalityOperation);
  if(data.application_form?.event_date){
    lifecycleEventDate.value=data.application_form.event_date;
    lifecycleNotes.value=data.application_form.notes || '';
  }
}
async function mortalityRequest(payload){
  const response=await fetch(`/api/pig-weights/pig/${encodeURIComponent(currentPigId)}/lifecycle/death`,{
    method:"POST",headers:{'Content-Type':'application/json','X-Mortality-CSRF':lifecycleForm.dataset.csrf},
    body:JSON.stringify(payload)});
  const data=await response.json();
  if(!response.ok||!data.success)throw new Error(mortalityText(data.answer)||
    (mortalityAf?'Die afsterwe is nie bevestig nie. Hersien die verslag en probeer weer.':'The death was not confirmed. Review the report and try again.'));
  return data;
}
async function submitLifecycleDeath(event){
  event.preventDefault();
  if(!currentPigId)return;
  if(lifecycleReason.value!=='Died'){
    if(!window.confirm(`Record ${lifecycleReason.value} on ${lifecycleEventDate.value}? The animal's history will be retained.`))return;
    setLifecycleSubmitting(true);
    try{
      const data=await mortalityRequest({reason:lifecycleReason.value,event_date:lifecycleEventDate.value,notes:lifecycleNotes.value});
      showMessage(data.message||'Lifecycle outcome recorded.','success');
      lifecycleForm.reset();
      await loadPigDetail();
    }catch(error){showMessage(error.message)}finally{setLifecycleSubmitting(false)}
    return;
  }
  setLifecycleSubmitting(true);
  renderMortalityPreview({});
  try{
    renderMortalityPreview(await mortalityRequest({phase:'preview',event_date:lifecycleEventDate.value,
      reason:'Died',notes:lifecycleNotes.value}));
  }catch(error){showMessage(error.message)}finally{setLifecycleSubmitting(false)}
}
async function confirmLifecycleDeath(){
  if(!mortalityOperation)return;
  setLifecycleSubmitting(true);
  try{
    const data=await mortalityRequest({phase:'confirm',operation_id:mortalityOperation});
    showMessage(mortalityText(data.answer),'success');
    renderMortalityPreview({});
    lifecycleForm.reset();
    await loadPigDetail();
  }catch(error){showMessage(error.message)}finally{setLifecycleSubmitting(false)}
}
async function resumeMortalityPreview(){
  if(!lifecyclePreview||!currentPigId)return;
  try{
    const response=await fetch(`/api/pig-weights/pig/${encodeURIComponent(currentPigId)}/lifecycle/death`);
    if(response.ok)renderMortalityPreview(await response.json());
  }catch(error){showMessage(mortalityAf?'Die vorige voorskou kon nie gelaai word nie.':'The previous preview could not be loaded.')}
}
if(lifecycleSubmitButton){
  lifecycleForm.addEventListener('submit',submitLifecycleDeath);
  lifecycleForm.addEventListener('input',()=>renderMortalityPreview({}));
  lifecycleConfirmButton.addEventListener('click',confirmLifecycleDeath);
}
updateBackLink();loadPigDetail().then(resumeMortalityPreview);
