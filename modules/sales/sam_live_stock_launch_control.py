...
def build_sam_live_stock_owner_review_packet(event, *, links=None, environ=None):
    ...
    keyboard.extend(_owner_card_prepared_action_buttons(decision, review_event_id or conversation_id))
...
def _owner_card_prepared_action_buttons(decision, callback_id):
    ...
    if not callback_id:
        return []
    status = _clean(packet.get("status"), 120)
    next_action = _clean(packet.get("next_action"), 120)
    internal_next_action = _clean(packet.get("internal_next_action"), 120)
    buttons = []
    if status == "ready_for_owner_prepare" or next_action == "prepare_draft_order" or internal_next_action in {"create_draft", "create_draft_then_quote"}:
