            ...
            with connection.cursor() as cursor:
                rows = _fetch_all_dicts(
                    ...
                    select
                        ...
                        st.sale_date,
                        st.sale_stream,
                        to_jsonb(st)->>'sale_channel' as sale_channel,
                        st.buyer_name,
                        st.destination,
                        st.linked_order_id,
                        st.pig_count,
                        ...
                        st.deductions_total,
                        st.net_total,
                        nullif(to_jsonb(st)->>'lot_total','')::numeric as lot_total,
                        to_jsonb(st)->>'financial_interpretation' as financial_interpretation,
                        nullif(to_jsonb(st)->>'received_total','')::numeric as received_total,
                        coalesce(nullif(to_jsonb(st)->>'receivable_total','')::numeric,
                                 st.net_total) as receivable_total,
                        coalesce(to_jsonb(st)->>'financial_disposition','Commercial') as financial_disposition,
                        nullif(to_jsonb(st)->>'output_vat','')::numeric as output_vat,
                        nullif(to_jsonb(st)->>'gross_including_vat','')::numeric as gross_including_vat,
                        nullif(to_jsonb(st)->>'commission_ex_vat','')::numeric as commission_ex_vat,
                        nullif(to_jsonb(st)->>'commission_input_vat','')::numeric as commission_input_vat,
                        nullif(to_jsonb(st)->>'commission_including_vat','')::numeric as commission_including_vat,
                        nullif(to_jsonb(st)->>'other_deductions','')::numeric as other_deductions,
                        nullif(to_jsonb(st)->>'net_settlement_payable','')::numeric as net_settlement_payable,
                        to_jsonb(st)->>'external_reference' as external_reference,
                        st.currency,
                        st.payment_status,
                        st.payment_method,
                        st.payment_date,
            ...
            with connection.cursor() as cursor:
                transaction_rows = _fetch_all_dicts(
                    ...
                    select
                        ...
                        receivable_total,
                        financial_disposition_evidence_sha256,
                        sale_status,
                        notes,
                        created_by,
                        created_at,
...
def get_monthly_sales_transaction_summary(report_date=None, database_url=None):
            ...
            with connection.cursor() as cursor:
                rows = _fetch_all_dicts(
                    ...
                    select
                        ...
                        sum(coalesce(st.receivable_total, st.net_total))::numeric(12, 2) as receivable_total,
    ...
    for row in rows:
        ...
        streams[stream_key] = {
            ...
            "receivable_total": _json_safe_value(row.get("receivable_total")),
    ...
    totals = {
        ...
        "receivable_total": round(sum(float(stream["receivable_total"] or 0) for stream in streams.values()), 2),
