...
def classify_message_intent(text: str, attachments: list[Mapping[str, Any]] | None = None) -> str:
    ...
    rules = [
        ...
        ("budgetary_quotation", r"\b(funding|finance approval|loan|budget approval|company approval|begroting|finansiering)\b"),
        ("current_availability_quotation", r"\b(current availability|available now|in stock|which pigs|beskikbaar nou)\b"),
        ("price_indication", r"\b(price|cost|how much|prys|hoeveel)\b|\br\s?\d+"),
        ("quotation_request", r"\b(quote|quotation|kwotasie)\b"),
