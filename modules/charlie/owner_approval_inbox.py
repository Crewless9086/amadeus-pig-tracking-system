...
def owner_approval_item_from_sam_meat_learning_event(event):
    ...
    captured = _json_dict(event.get("captured_facts_json"))
    wanted = _json_dict(event.get("customer_wanted_json"))
    missing = _json_list(event.get("missing_facts_json"))
    objections = _json_list(event.get("objections_json"))
    confusion = _json_list(event.get("confusion_signals_json"))
    misses = _json_list(event.get("sam_misses_json"))
    reply = _clean(event.get("sam_reply_excerpt"), 2500)
    suggestion = _clean(event.get("improvement_suggestion"), 600)
    message = _clean(event.get("customer_message_excerpt"), 500)
    lead_id = _clean(event.get("lead_id"), 120)
    ...
    if lead_id:
        ...
        title_parts.append(conversation_id)
    item = normalize_owner_approval_item({
        ...
        "status": "pending",
        "action_label": _sam_meat_learning_action_label(event),
        "exact_action": (
            "Review the saved SAM Meat conversation learning evidence. This inbox card is "
            "read-only; use the meat sales/customer-send or money-path gate for any action."
        ),
        "exact_text": reply or suggestion,
        "editable_text": reply or suggestion,
        "target_label": lead_id or conversation_id,
        "source_ref": conversation_id or lead_id,
        "conversation_id": conversation_id,
        "risk_flags": _sam_meat_learning_risk_flags(event, missing, objections, confusion, misses, captured, wanted),
        "forbidden_actions": [
            "Inbox approval does not send a customer message or change SAM prompts/rules.",
            "No quote, invoice, order, payment, stock, reservation, Chatwoot, public-post, or farm lifecycle write from this card.",
        ],
...
def _sam_meat_learning_action_label(event):
    event_type = _clean(event.get("event_type"), 80).replace("_", " ")
    signal = _clean(event.get("conversion_signal"), 80).replace("_", " ")
    if signal and signal != "unknown":
        return f"Review SAM Meat {signal}"
    if event_type:
        return f"Review SAM Meat {event_type}"
    return "Review SAM Meat learning evidence"


def _sam_meat_learning_risk_flags(event, missing, objections, confusion, misses, captured, wanted):
    flags = ["evidence_only_learning"]
    signal = _clean(event.get("conversion_signal"), 80)
    if signal:
        flags.append(f"conversion:{signal}")
    event_type = _clean(event.get("event_type"), 80)
    if event_type:
        flags.append(f"event:{event_type}")
    ...
    for key in ("product_type", "cut_set", "location", "delivery_or_collection", "timing", "payment_method"):
        value = captured.get(key) or wanted.get(key)
        if value:
            flags.append(f"{key}:{_clean(value, 80)}")
    return flags
