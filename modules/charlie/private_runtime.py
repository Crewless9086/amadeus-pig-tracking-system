...
def handle_private_telegram_webhook(payload, headers=None, *, environ=None, sender=None, callback_answerer=None, store=None):
    ...
    try:
        if callback:
            ...
            result, status = _handle_message(payload, binding, store, sender, environ)
...
def _handle_message(payload, binding, store, sender, environ):
...
def _approval_for_intent(thread_id, intent, store):
    ...
    result, _ = store.create_approval_bundle(thread_id, "CHARLIE owner decision", f"Requested action: {intent['type']}", [{"intent": intent["type"], "args": intent.get("args") or {}}], {"recommended": "review_then_confirm"}, state_hash)
    return result
...
def _reply(binding, text, store, sender, update_id, media, environ, *, reply_markup=None, status_code=200):
    ...
    store.record_message(binding["thread_id"], "charlie", text, update_id=f"reply-{update_id}", media=media, metadata={"send_status": sent.get("status")})
    return {
