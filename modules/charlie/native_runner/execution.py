...
RUNTIME_BOUNDARY = "charlie-native-runner"
...
def _host_role_identity(result, *, task, purpose, schema_name, prefix):
    ...
    if (audit.get("runtime_boundary") != RUNTIME_BOUNDARY or audit.get("task") != task
    ...
    material = json.dumps({
        "runtime_boundary": RUNTIME_BOUNDARY, "task": task, "provider": provider,
    ...
    def ensure(self):
        if self.worktree_root.exists():
            ...
            if committed.returncode != 0:
                raise NativeExecutionError("native_git_identity_failed")
            committed_paths = {
                normalize_repo_path(item) for item in committed.stdout.splitlines() if item.strip()
            }
            if (origin != REMOTE or branch != self.authorization.branch
                    or ancestor.returncode != 0
                    or not changed_paths.issubset(self.authorization.allowed_files)
                    or not committed_paths.issubset(self.authorization.allowed_files)):
...
class NativePackager:
    ...
    def _validate_candidate_before_remote(self, head):
        ...
        summary = self._git("diff", "--summary", self.authorization.starting_main_sha, head, "--")
        lowered = summary.lower()
        if any(marker in lowered for marker in (
                "mode change", "create mode 120000", "rename ", "delete mode")):
            raise NativeExecutionError("native_candidate_metadata_change_rejected")
        patch_text = diff.stdout if isinstance(diff.stdout, str) else diff.stdout.decode("utf-8", "replace")
        if "GIT binary patch" in patch_text or "Binary files " in patch_text:
            raise NativeExecutionError("native_candidate_binary_rejected")
        return candidate_files, diff
