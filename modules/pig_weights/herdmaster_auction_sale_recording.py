    ...
    try:
      with connect_factory() as connection:
       with connection.cursor() as cur:
        ...
        received="4470.51" if fresh["payment_status"]=="Received" else None
        payment_status="Paid" if received else "Unknown"
...
def reconcile_auction_payment(sale_id, evidence, *, authority, authority_verifier=None, connect_factory=None, database_url=None):
    ...
    try:
      with connect_factory() as connection:
       with connection.cursor() as cur:
        cur.execute("select pg_advisory_xact_lock(hashtextextended(%s,0))",("herdmaster-auction-payment:"+str(sale_id),))
        cur.execute("select sale_channel,net_settlement_payable,received_total,payment_received_evidence_json from sales_transactions where sale_id=%s for update",(sale_id,))
        row=cur.fetchone()
        ...
        if row[2] is not None:
            if str(row[2])=="4470.51" and row[3]==identity: return _result(True,"payment_replayed_zero_rows",rows_changed=0),200
            return _result(False,"payment_reconciliation_conflict"),409
        cur.execute("update sales_transactions set received_total=4470.51,payment_status='Paid',payment_date=%s::date,payment_received_evidence_json=%s::jsonb,updated_at=now() where sale_id=%s and received_total is null",(identity["received_date"],json.dumps(identity,sort_keys=True),sale_id))
