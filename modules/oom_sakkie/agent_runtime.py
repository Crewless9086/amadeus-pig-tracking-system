...
CURRENT_CLAUDE_REVIEW_FOCUS = [
    ...
    "Sales campaign rail persists Ledger opportunities as append-only owner-review campaign records, but has no customer-send, quote/order, stock-change, dispatch, runtime, prompt, public-output, or physical-control consumer.",
    "Sales campaign approval now records an append-only owner event and queues an internal customer outreach draft only; /approve_campaign and /drafts remain owner-only and do not send to customers, call Chatwoot/n8n, create quotes/orders, reserve/change stock, or dispatch agents.",
    "Ledger Sales Workbench now surfaces campaigns, outreach drafts, Sam/Chatwoot send-design requests, and sales lead tracking in the local web app; send-design and lead records are append-only review records only and cannot send customers, call Chatwoot/n8n, create orders, reserve/change stock, or dispatch agents.",
...
_AGENT_TOOLS = {
    ...
    "ledger": ("business_growth_brief", "sales_offer_brief", "sales_customer_draft", "ledger_sales_agent", "sales_campaign_status", "sales_outreach_draft_queue", "sales_send_design_status", "sales_lead_tracking_status", "sales_dashboard", "meat_planning"),
    "atlas": ("farm_operating_brief", "dashboard_summary", "pig_allocation_readiness", "power_recent"),
    "rootline": ("rootline_water_energy_plan", "weather_now", "weather_today", "weather_forecast", "irrigation_status"),
...
_TOOL_PRIMARY_AGENT = {
    ...
    "sales_campaign_status": "ledger",
    "sales_outreach_draft_queue": "ledger",
    "sales_send_design_status": "ledger",
    "sales_lead_tracking_status": "ledger",
    ...
    "rootline_water_energy_plan": "rootline",
