...
RULES = [
    ...
    (
        re.compile(r"\b(daily command brief|jarvis daily brief|oom sakkie daily brief|command brief|start my day|daily command|run the command brief|give me the command brief|full jarvis brief)\b", re.I),
        IntentMatch("jarvis_daily_command_brief", "jarvis_daily_command_brief", 0.95, "rule:jarvis_daily_command_brief"),
    ),
    (
    ...
    (
        re.compile(r"\b(agent authority matrix|authority matrix|agent authority|agent permissions|agent powers|what authority.*agents|what can agents control|which agent powers are locked)\b", re.I),
    ...
    (
        re.compile(r"\b(dispatch runtime review packet|dispatch review packet|dispatch execution review|review dispatch runtime|claude.*dispatch.*review|dispatch.*claude.*review)\b", re.I),
        IntentMatch("dispatch_runtime_review_packet", "dispatch_runtime_review_packet", 0.95, "rule:dispatch_runtime_review_packet"),
    ),
    (
    ...
    (
        re.compile(r"\b(sales campaign status|sales campaigns|campaign queue|sales queue|customer outreach queue|what sales campaigns|campaigns waiting)\b", re.I),
        IntentMatch("sales_campaign_status", "sales_campaign_status", 0.95, "rule:sales_campaign_status"),
    ),
    (
        re.compile(r"\b(outreach drafts|customer outreach drafts|draft queue|sales draft queue|customer drafts waiting|outreach draft queue)\b", re.I),
        IntentMatch("sales_outreach_draft_queue", "sales_outreach_draft_queue", 0.95, "rule:sales_outreach_draft_queue"),
    ),
    (
        re.compile(r"\b(send design|customer send design|sam handoff design|chatwoot handoff design|send design queue|customer send queue)\b", re.I),
