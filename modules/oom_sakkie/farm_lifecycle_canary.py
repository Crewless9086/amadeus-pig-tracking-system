...
def _herdmaster_packet(evidence, observation_pig_id):
    ...
    source_names = {str(row.get("name") or "").strip() for row in sources if isinstance(row, dict)}
    ...
    if missing_sources:
        gaps.append("Herdmaster evidence must cite canonical sources: " + ", ".join(missing_sources) + ".")
    if not events:
...
def _state_event_conflicts(state, events):
    if not isinstance(state.get("on_farm"), bool):
        return []
    event_types = {str(event.get("lifecycle_event_type") or "").strip() for event in events if isinstance(event, dict)}
    if state["on_farm"] and "exited_farm" in event_types:
        return ["Current state says on_farm=true but lifecycle evidence records exited_farm."]
    if not state["on_farm"] and "entered_farm" in event_types:
