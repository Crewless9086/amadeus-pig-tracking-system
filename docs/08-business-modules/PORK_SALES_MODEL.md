...
- Sam Meat payment instructions are environment-configured and may be sent after the customer accepts an owner-approved follow-up. Bank details are shared across sales streams through `BANK_ACCOUNT_NAME`, `BANK_NAME`, `BANK_ACCOUNT_NUMBER`, `BANK_BRANCH_CODE`, and `BANK_ACCOUNT_TYPE`; the older `MEAT_SALES_BANK_*` names remain fallback-only. Customer bank references are short, stable sale suffixes of six alphanumeric characters or fewer, for example `ORD-2026-A99273` uses reference `A99273`. Customer POPs are logged as `pop_received_unverified`; only bank-confirmed money (`deposit_confirmed_in_bank`) unlocks slaughter, butcher, and delivery gates.
