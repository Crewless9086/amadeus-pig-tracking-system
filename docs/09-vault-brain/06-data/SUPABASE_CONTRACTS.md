...
## Canonical Farm Import Integrity

- Preserve source sheet row, source record identity and import batch identity
  for every imported or excluded record.
- A missing animal identity is never guessed. Exclude it from canonical events
  and retain it in a review/quarantine output.
- Identical same-animal/date/value weight duplicates collapse to one canonical
  event while every source reference remains auditable.
- Repeated identical movement events collapse to one canonical movement while
  every source reference remains auditable.
- Conflicting same-animal/date weight values remain excluded from canonical
  weight/current-state, readiness, allocation and valuation calculations until
  resolved through a governed review. Review evidence must show the animal,
  date, candidate values, source rows/record IDs and decision status.
- Import retries are idempotent and batch-bound. Rollback targets only the exact
  import batch in reverse dependency order and never rewrites the source sheet.

Historic review counts and row totals belong in archived evidence or fresh
runtime reports, not reusable doctrine. Current unresolved items must be read
from the canonical review mechanism or regenerated by the approved verifier.
