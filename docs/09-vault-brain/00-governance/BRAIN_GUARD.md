...
7. BEACON/Meta livestock-awareness missions include
   ...
   the former `live_stock_enquiry_capture` exception is retired: disclaimers do
   not authorize buyer targeting, acquisition-detail solicitation or commercial
   contact calls-to-action; every card and execution must bind the common
   evaluator plus exact current surface/entity/jurisdiction authority evidence;
