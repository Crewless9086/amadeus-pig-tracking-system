...
### Mechanically enforced mission outcome contract

CORE must parse `core_mission_outcome_handover_v1` at the canonical mission
boundary. Terminal prose, Markdown length, tests, PR, merge, deployment, health
or a technical `done` status never changes the business lifecycle. A terminal
may report only a stage-qualified `terminal_disposition`; it cannot request or
set `BUSINESS_COMPLETE`. Bare `complete`, `completed` and `done` dispositions
are invalid. Missing, malformed, cross-mission or replay-conflicting contracts
produce `INVALID_HANDOVER`, append the evaluation where identity is valid, and
leave the prior open lifecycle unchanged.
