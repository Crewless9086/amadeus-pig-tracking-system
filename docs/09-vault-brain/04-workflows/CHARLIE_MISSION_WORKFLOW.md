...
Authenticated Telegram mission callbacks claim their `update_id` before mutation and complete a durable sanitized outcome record. Final-release callbacks must bind to the current review generation and atomically compare both that generation and `pr_ready`; stale, duplicate, invalid, and non-owner callbacks fail closed without changing mission authority.
