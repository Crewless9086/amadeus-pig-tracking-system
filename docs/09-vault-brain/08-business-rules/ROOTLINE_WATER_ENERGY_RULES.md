...
Last owner correction: 2026-08-05
...
- The intended controllable device is `BOREHOLE PUMP (1)`, but its IFTTT ON/OFF binding has not yet been created or entered into canonical device data.
- Until that exact binding is created and physically proven, ROOTLINE may plan and recommend borehole windows but must not claim or attempt control.
...
- Once the binding is governed, nighttime is the borehole catch-up window. ROOTLINE chooses start time and duration from water continuity, storage/reservoir evidence, actual rain, forecast reliability, battery/solar/grid evidence, and expected need.
...
- ROOTLINE may run B only, C only, both sequentially, both concurrently when the proven hydraulic/electrical system permits it, shorten/extend a run, move it, or hold it. The plan must state what will run, when, for how long, why, and when it will reassess.
...
- Energy policy is device-class specific. It governs material electrical loads such as the future borehole pump and fertilizer mixer, not passive water delivery through the B/C gravity-fed valves. ROOTLINE must never apply a generic farm-energy gate to every registered device.
...
For initial B/C autonomy, the owner-approved standing operating envelope is deliberately narrow: proven B and C irrigation valves only; no fertilizer or borehole actuation; no concurrent B/C run until hydraulic capacity is separately proven; each native fail-stop segment is at most 60 minutes; a nominal two-hour objective requires two distinct segments with verified shutdown and a fresh canonical decision before segment two. ROOTLINE may plan and execute eligible B/C segments within this envelope without requesting routine per-run permission. A genuine boundary exception, uncertain shutdown, unavailable fail-stop, conflicting evidence or proposed expansion beyond this envelope requires containment and owner attention.
...
- B/channel 1 and C/channel 2: gravity-fed irrigation valves; `minor_control_only`; farm SOC/reserve is excluded from irrigation need, ranking, timing and execution eligibility.
- Future borehole plug/pump: `material_load`; water continuity, storage/reservoir state, night catch-up, SOC, solar, grid and forecast reliability are relevant, with water continuity allowed to outweigh grid avoidance.
...
Creating an IFTTT function does not prove a physical device mapping and does not grant ROOTLINE actuation authority.
...
These rules authorize observation, reasoning, adaptive planning, owner summaries and, after the reviewed autonomous execution mission is integrated and operationally proven, standing B/C execution within the narrow owner-approved envelope above.
...
- ambiguous ON retry, simultaneous B/C, automatic segment-two continuation without fresh decision, or execution beyond 60 minutes without an independently reviewed fail-stop;
...
Standing B/C authority becomes operational only after a separately reviewed implementation binds the existing commissioned device evidence, automatic scheduler, deterministic state-setting ON, native 60-minute auto-OFF, safe repeated OFF, shutdown verification, daily plan lifecycle and exception containment. This owner decision removes routine per-run approval; it does not waive those technical prerequisites or expand authority to other devices.
