...
|---|---|---|---|
| CORE | Application preview adapter source-ready; current-main reconciliation required | PR #893 exact head `988f62ac` has green CI and independent GO reviews, but is based on `07e2dbc8` while main is `9300ca3f`; the intervening UIQ files are disjoint. It changes only the read-only application preview path and leaves executors/staging/OOM/UI/fallbacks unchanged. | `CONTINUE`: reconcile the existing PR onto current main, repeat exact-head evidence, then use the serialized release lane to merge/deploy and prove one authenticated side-effect-free application preview with zero staging, executor or farm effects. |
...
- `CMQ-20260813-03` — `application_adapter_source_ready_reconcile_release`:
  PR #893 exact head `988f62ac` is green/reviewed but behind current main. Its
  file scope is disjoint from UIQ #894. Reconcile, re-review, then integrate and
  prove the authenticated preview endpoint read-only; staging/execution, OOM,
  UI, migrations and fallbacks remain excluded.
