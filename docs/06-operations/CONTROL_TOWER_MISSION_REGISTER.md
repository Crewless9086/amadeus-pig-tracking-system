...
| HERDMASTER | Lifetime genetic-merit backend PR ready for integration | Contract PR #902 is integrated. Backend PR #905 exact head `130741df` adds a repeatable-read Supabase snapshot, versioned herd/named-animal composer and owner-protected read-only endpoints. All required checks and specialist reviews pass; current-main advance is file-disjoint. No deployment or owner-visible result exists yet. | Reconcile normally onto current main, integrate/deploy through the serialized lane and prove authenticated genuine readback plus unchanged farm tables. Keep CODEX UI blocked until the runtime contract is proven. |
| ROOTLINE | Callback defect repaired; fresh supervised journey required | Exact trace proved message `3586` reached the authenticated callback and atomic claim, then stopped at `protected_irrigation_eligibility_changed` because volatile fresh-readback digests were compared with preview-time eligibility. PR #904 merged as `406a6851` and is deployed in successor `1e6846d7`; transport timeout was raised without adding n8n authority. The old claim is contained and today has new job/segment identities. Zero controls occurred. | `WAIT FOR FRESH OWNER PRESENCE`: do not reuse message `3586`. On explicit current valve-side presence, recompute eligibility and issue exactly one new short-lived digest-bound preview; then observe the genuine confirmation through provider, Supabase, coordinator, controller and physical evidence. |
...
- `HMQ-20260813-04` — `backend_pr905_ready_for_integration`: reconcile
