...
|---|---|---|---|
| CORE | Application preview operational; OOM preview adapter slice cleared | PR #893 merged/deployed as `de4122ab`. Two authenticated production movement-only previews returned the same canonical digest with confirmation required and unchanged canonical table counts; no staging, executor, provider or farm effect occurred. Wider executor cutover remains incomplete. | `NEW MISSION`: wire only the existing OOM typed grouped-weight preview runtime to the canonical preview contract in disjoint source/tests. Preserve OOM execution/protected claims, application adapter, UI, voice, schemas and fallbacks; no merge/deploy yet. |
...
| ROOTLINE | Multi-segment correction review-ready; reconcile and release | PR #891 exact head `6c81edd6` now preserves the 7,200-second plan, separates two 3,599-second executable segments, loads Supabase job history and proves atomic claims/replay suppression. All gates and hardware/persistence/authority reviews pass. Main advanced to `de4122ab` through file-disjoint CORE PR #893. B/C remains disabled and zero controls occurred. | `CONTINUE`: reconcile the same PR onto current main, renew exact-head evidence, merge/deploy through the serialized lane, prove loaded persistence and controller safety read-only, then present one protected B acceptance preview only if the full current contract passes. No actuation before Charl confirms. |
| OOM SAKKIE | Health/loss Stage 1 integrated; Stage 2 runtime readiness active | Workflow PR #892 and zero-I/O evaluator PR #895 are merged, the latter as `6fd4d271`; source worktree is released. Gateway/runtime source exists, but deployed activation, loaded configuration, canonical evidence completeness and provider delivery remain unproven. Pig 002 remains failed-acceptance evidence only. | `NEW MISSION` within the same outcome: audit authenticated routing/evidence loading and deployed configuration read-only; correct only reusable Stage 2 defects through review/deployment, then prove a non-writing authenticated runtime preview family without replaying Pig 002 or asking Charl for another message until readiness is established. |
...
- `CMQ-20260813-03` — `application_preview_operational_oom_adapter_cleared`:
  PR #893 is deployed as `de4122ab`; repeated authenticated previews produced
  one stable digest and zero canonical-table or external effects. Clear only
  the OOM typed preview adapter as disjoint source/tests; executor, protected
  claims, application, UI, voice, migrations and fallbacks remain excluded.
...
- `HERDMASTER-NATURAL-HEALTH-LOSS-1/OOM-INTAKE-SLICE-1` —
  `stage1_integrated_stage2_runtime_readiness`: workflow PR #892 and evaluator
  PR #895 (`6fd4d271`) are integrated. Next prove authenticated gateway routing,
  runtime-loaded configuration, complete canonical evidence and zero-write
  preview delivery. Pig 002 must not be replayed or written.
