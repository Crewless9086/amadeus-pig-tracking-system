...
|---|---|---|---|
| CORE | Shadow blocked by private mission creator; portfolio baseline added | Canonical lookup confirms no CMQ-05/alias and zero Shadow events. The private creator drops the approved opaque ID. Owner also approved epoch `CORE-CURRENT-2026-08-14`: legacy status/evidence stays immutable, only admitted missions become schedulable, findings remain parent tasks unless promoted, and three boards derive from canonical truth. | Finish/review/deploy exact-ID repair first. Then create/admit exactly one CMQ-05 and produce the no-write legacy/current-admission manifest for Charl's approval. Do not start ten Shadow comparisons, mutate legacy rows or restart the runner before portfolio enforcement/admissions are approved. |
...
- `CMQ-20260813-05` — `blocked_creator_repair_then_portfolio_baseline`: Phase A
  Shadow is deployed/enabled but cannot persist until the private creator preserves
  the approved opaque ID. After that repair, create exactly one CMQ-05 admitted to
