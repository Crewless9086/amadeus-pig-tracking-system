...
|---|---|---|---|
| CORE | CMQ-03 deployed/acceptance pending; Shadow Control Tower ready to start | PR #899 merged/deployed as `84601c83`; loaded revision truth and cross-domain tests prove exclusive ROOTLINE-irrigation versus grouped-weight executor dispatch, with zero claims, sends, controls or farm writes during integration. CMQ-03 remains open only for a later genuine protected journey. The visible CORE terminal is released and owner-authorized `CMQ-20260813-05` Phase A is now eligible. | `START CMQ-20260813-05`: one visible CORE terminal builds source-only Shadow Mode on the existing durable CORE queue/runner/register. No hidden terminals, live dispatch, merge/deploy or production mutation. Record proposed decisions and compare ten later real Control Tower transactions before any authority promotion. |
| HERDMASTER | Lifetime genetic-merit contract ready for integration | HMQ-20260813-04 PR #902 at `f831288d` is a reviewed documentation-only data/UX contract on current main `693c11f7`; all required checks and specialist reviews pass. It preserves Unknown, separates association from causation, defines confidence/sample-size rules and assigns backend semantics to HERDMASTER and rendering only to CODEX UI. No implementation or Business outcome exists. | Integrate PR #902 normally, then keep HMQ-04 on the visible HERDMASTER terminal for one isolated backend read-model/composer slice. Do not assign CODEX UI files until the versioned read contract is implemented and reviewed. |
| ROOTLINE | Genuine B confirmation failed; read-only callback trace required | PR #900 merged/deployed as `693c11f7` and remains in deployed successor lineage. Telegram preview `3586` was delivered for B/CH1 segment 1, and Charl pressed `Bevestig`, but observed no response or irrigation start. The report predates the press, so provider callback receipt, Supabase claim transition, execution dispatch and controller truth after the press are not yet known. | `CONTINUE RMQ-20260813-04 — SEND/OPERATE NOTHING`: preserve message `3586` and trace its exact callback end to end read-only. Do not ask Charl to press again or actuate manually. Repair only the reusable systemic boundary exposed by the retained event. |
...
- `CMQ-20260813-05` — `phase_a_shadow_ready_for_dispatch`: build observation-only
...
- `HMQ-20260813-04` — `contract_ready_backend_slice_next`: reconcile
...
- `RMQ-20260813-04` — `failed_acceptance_callback_trace`: prior physical B/C
