...
|---|---|---|---|
| CORE | Shadow enabled but blocked by missing canonical mission | PR #906 is deployed and observation flag enabled with all authority counters zero. The first genuine proposal attempt failed closed with authenticated 404 because `CMQ-20260813-05` exists only in the Control Tower register and not in canonical Supabase mission truth; zero events/comparisons remain. | Create/reconcile exactly one owner-approved CMQ-05 mission through the existing canonical private mission-creation spine, never by bypass. Verify exact readback and zero duplicates, then retry the original SHADOW READY proposal before human assessment. |
...
- `CMQ-20260813-05` — `blocked_missing_canonical_supabase_mission`: build observation-only
