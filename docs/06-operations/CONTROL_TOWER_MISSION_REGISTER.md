...
|---|---|---|---|
| CORE | Shadow blocked by private mission creator dropping opaque ID | Canonical lookup confirms no CMQ-05 or alias row and zero Shadow events. The authenticated `create_mission` action accepts only title/raw text and calls `record_mission()` without the explicit owner-approved `mission_id`, which would generate/reuse the wrong identity. CORE correctly performed no mutation. | Repair the existing private mission-creation action to preserve an explicitly authorized opaque ID end to end, cross-referencing CMQ-02A. Review/deploy first; only then create exactly one CMQ-05 row, prove replay/readback and retry the still-undecided Shadow proposal. |
...
- `CMQ-20260813-05` — `blocked_creator_drops_opaque_mission_id`: build observation-only
