...
| ROOTLINE | Card-lifecycle repair/governance proven; waiting only for fresh presence | At authoritative/deployed `d527a560`, the Standard and Programme preflight passed completely. PR #907 gives each preview a digest-scoped card identity, exact callback/card/claim binding and atomic containment of unprovable delivery; retry cannot resend/re-execute. Fresh 08:36 SAST provider readback is online/all OFF with B fail-stop 3,599 seconds and zero controls. | `WAIT FOR FRESH OWNER PRESENCE`: no development work. On one new explicit current valve-side statement, recompute eligibility and create exactly one fresh short-lived card; retain RMQ-04 through physical segment/shutdown proof. |
...
- `RMQ-20260813-04` — `ready_waiting_for_fresh_presence`: prior physical B/C
