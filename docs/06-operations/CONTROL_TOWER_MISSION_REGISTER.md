...
| 2026-08-17 | CORE `CMQ-20260813-05` isolated staging rail integrated | PR #1031 exact head `b55ed425` implemented an HMAC-sealed fixed-issuer validation receipt, exact immutable SourceRef, independently supplied scheduled-task digest, serialized non-stealable lane, authoritative non-overlapping runtime/execution roots, governed-stop preservation, manifest/worktree TOCTOU protection, explicit digest-bound recovery and deterministic rollback containment. Twenty-seven safe local source tests, static compilation, all three hosted CI lanes and independent process-safety/security and release-integrity reviews passed; no local runner/process-control test or runtime mutation occurred. Control Tower integrated the source-only rail as authoritative main `ebc661b5` after first merging the durable blocker record. | Promote CMQ-05 to `WORKING / STAGING_RAIL_INTEGRATED / ISOLATED_STAGING_AUTHORITY_REQUIRED`; retain Recovery Slot 1 and Business completion false. Authorize one isolated zero-start staging epoch for exact SourceRef `ebc661b5`: provision protected receipt authority with restrictive ACL, generate a sealed exact-revision validation receipt from an independent validation boundary, establish lane ownership and rollback tuple, and stage runtime/execution/manifest agreement while preserving `supervisor.stop` and leaving the watchdog unregistered/unmodified and CORE stopped. Fail closed on any changed key, receipt, task digest, dirty root, contradiction or concurrent lane. No runner start/stop, process enumeration/termination, scheduled-task enablement, Supabase/provider/farm action or Business completion claim. Return the full handover before any activation decision. |
