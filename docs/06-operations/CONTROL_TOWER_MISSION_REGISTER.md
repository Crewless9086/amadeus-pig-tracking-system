...
|---|---|---|---|
| CORE | Opaque creator PR #910 ready; portfolio baseline remains next gate | PR #910 exact head `0811d775` on current main `8a4fb58c` preserves explicitly authenticated opaque IDs, rejects aliases/substitution/title collisions and serializes exact/legacy creation races. All CI and three reviews pass; no mission/event/runtime mutation occurred. Owner-approved epoch `CORE-CURRENT-2026-08-14` remains planned. | Integrate/deploy #910 first and prove loaded exact-ID behavior without creating a mission. Then create/admit exactly one CMQ-05 and produce the no-write legacy/current-admission manifest. Do not begin Shadow scoring, legacy mutation or runner restart. |
