...
|---|---|---|---|
| CORE | Pure grouped-preview contract integrated; one adapter slice cleared | `CMQ-20260813-03` PR #890 merged normally as `a9ad1d1a`; exact-merge tests/checks passed and the released worktree is clean. The canonical contract remains unwired and produces no runtime or business change. | `NEW MISSION`: from a fresh current-main worktree, wire only the existing application grouped-preview endpoint to the canonical contract, preserving the application executor/staging path unchanged. Source and tests only; no deployment, UI, OOM adapter, migrations or fallback retirement. |
...
| ROOTLINE | Multi-segment source active; PR #891 not integration-ready | Historical B/C commissioning remains valid. PR #891 exact head `abe6a040` is mergeable and all three CI gates are green, but its ordinary eligibility default still caps requested total runtime at one 3,599-second segment and rebuilds projection from an empty event list, so durable segment-2 advancement is not yet proven. B/C autonomy remains disabled and zero controls occurred. | `ADDENDUM`: keep RMQ-04 in the existing ROOTLINE worktree; do not merge or actuate. Bind requested total runtime and canonical persisted job/segment history into eligibility/coordinator projection, then obtain exact-head safety and persistence reviews. |
| OOM SAKKIE | Health/loss intake blocked on missing controlling workflow | `HERDMASTER-NATURAL-HEALTH-LOSS-1`: Pig 002 remains immutable failed-acceptance evidence. Authoritative main already contains the pure evaluator, tests, preview adapter, runtime and source handover, but the owner-approved controlling workflow exists only in historical commit `d78f1e02`; PR #650 was closed and did not itself contain that workflow file. | `CONTINUE`: use the retained clean OOM worktree for one reviewed documentation-only reconciliation of the workflow against current-main implementation truth. Charl's current handover reaffirmed the existing mission and canonical workflow, so do not request repeat approval or substitute fallback doctrine. After merge, repeat preflight and audit existing stage-1 source before writing anything; no replay, route activation, provider action or farm write. |
...
- `CMQ-20260813-03` — `contract_integrated_application_adapter_cleared`: PR
  #890 merged as `a9ad1d1a` and the exact merge passed. Clear only the existing
  application grouped-preview endpoint adapter in a new current-main worktree;
  keep staging/execution, OOM, UI, migrations and fallbacks unchanged and make
...
- `HERDMASTER-NATURAL-HEALTH-LOSS-1/OOM-INTAKE-SLICE-1` —
  `governance_reconciliation_required`: controlling workflow is absent from
  authoritative main, while the evaluator, tests, preview adapter, runtime and
  source handover already exist. Restore and update the owner-approved workflow
  through a documentation-only PR, then audit rather than duplicate stage 1.
  Pig 002 must not be replayed or written.
