...
configured Afrikaans, retained transcript/provenance and existing clarification,
...
append explicit weaning and reverse by removing only that addition after exact
state checks. Private actor/chat, farm_manager role, af, treatment and all other
settings are preserved in the prepared before/after hashes and private values.
Nothing has been applied. Scheduler acceptance must precede any binding change.
...
Evidence: root .tmp/control-tower-farm-release-review-20260912/, particularly
PUBLISHED_PR.json, HOSTED_REVIEW_RESULT_FIRST.json, MIGRATION_BYTE_IDENTITY.json,
ANTON_WEANING_PERMISSION_PREPARATION.json, voice-audit/AUDIT_RECEIPT.json,
SCHEDULER_WATCH_RUNNING.json and CONSOLIDATED_PRODUCTION_DECISIONS.md. The prior
pending farm-register addition is now recorded here with current approval and
actual hosted results. Disposition CONTINUE; no business outcome is closed.
