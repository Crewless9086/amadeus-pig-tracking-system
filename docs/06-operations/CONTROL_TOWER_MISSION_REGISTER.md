...
|---|---|---|---|
| CORE | Grouped-weight compatibility active; Shadow Control Tower authorized in parallel | CMQ-03 remains the existing canonical-preview/executor compatibility mission. Owner authorized `CMQ-20260813-05` Phase A Shadow Control Tower: reuse the existing mission queue, runner and register; one visible CORE terminal builds it and one durable existing CORE backend evaluates events. It may not spawn hidden terminals, dispatch prompts, merge, deploy or mutate production. | `PARALLEL MISSION`: keep CMQ-03 file scope untouched. Build source-only Shadow Mode with durable proposed-decision/evaluation records and a kill switch; compare ten later real Control Tower transactions before any authority promotion. |
| HERDMASTER | Lifetime genetic-merit mission promoted for analytics contract | HMQ-02/HMQ-03 remain complete. Existing PR #823/worktree `herdmaster-lifetime-genetic-merit-stages1-3` contains four unique commits and is 239 commits behind main; the plan-only worktree contains one overlapping unique commit and is 244 behind. Live `/breeding-analytics` remains narrow mating/litter reporting with unsafe Unknown handling and no full-lifecycle interpretation. | `NEW MISSION HMQ-20260813-04`: read-only reconcile both retained worktrees, production schemas/read models and live page into one page-specific data/UX contract. Do not revive/merge stale source or start UI implementation yet. |
...
- `HMQ-20260813-04` — `active_readonly_contract_reconciliation`: reconcile
