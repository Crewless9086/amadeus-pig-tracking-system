# Control Tower register through 2026-09-18

Status: historical evidence; non-doctrine.
Lifecycle: historical.

Preserved on 2026-09-19 from commit `c4b846cc86a63c98c0b83c2a1a1ad73a03afa5a0`, path `docs/06-operations/CONTROL_TOWER_MISSION_REGISTER.md`.
Original tracked payload SHA-256: `7ab08c20a7e2fb9b02c82cb2a352090053a96bad08181c594d30d2d8364b4401`.
...
## 2026-09-18 22:02 SAST - PR #1340 qualified; admission succession held

Approved publication completed: draft PR #1340,
`https://github.com/Crewless9086/amadeus-pig-tracking-system/pull/1340`, is exact
head `deff39235adf357ab19ac971804addff1171188e` on
`codex/oom-shutdown-visibility-20260918`, base `e46743cb...`. It remains draft,
open, unmerged and BLOCKED. GitHub's generated test merge `d819f966...` has the
exact reviewed source tree `618d4d933b3d127498b65bbb45c53b9aac7e6073`; this is
not a merge into main.

All application qualification jobs passed: `charlie-core`, disposable
PostgreSQL audit (including 272 herd tests / 27 subtests and later dialogue and
browser gates), closed disposable migration rail (38 tests / 56 subtests), and
the separate Playwright behavior gate. The three admission-related checks
failed because there is no exact external receipt marker. Admission test steps
passed before the receipt-presence gate; failed checks were not rerun or hidden.

Configured read-only SQL at 21:58/22:01 proved the root OMQ mission still has
valid admission for PR #1334/head `77227a67...`, and MORNING-CONTAINMENT still
has valid admission for PR #1336/head `23130559...`. Neither binds PR #1340 or
the Desktop branch. These valid admission projections are distinct from the
consumed prior release approval. No admission or business record was changed.

Independent read-only route inspection did not verify a truthful supported
Desktop candidate-succession operation preserving full previous packets and
authority history. Generic rebinding conflicts with valid old admissions and
projects Hermes identity; the same-PR invalidator does not describe a different
PR, and the Slack pilot does not cover this Desktop application scope. Do not
use those methods as shortcuts or run the protected issuer blindly. The next
technical prerequisite is a supported, exact, history-preserving Desktop
succession contract, followed by a reviewable admission-only authority proposal.

Current publication/qualification handover and full Check Receipt:
`control-tower-artifacts/OMQ-20260813-03/desktop-20260918/publication/PR1340_RECEIPT.md`.
The original admission diagnosis and exact binding material are retained under
the same publication artifact directory. Local documentation mirrors use short
filenames where needed, with hash-verified mapping in `evidence-index.json`.

This completes the owner-approved publication/hosted-check phase, not mission
OMQ-20260813-03 or its owner outcome. Main and production remain unchanged;
all 59 inherited worktrees and 96 PR heads remain, with only draft #1340 added
(97 open PRs, 62 worktrees). Former coordinator remains inactive. Both source
and review agents are released-retain. No admission write/callback, merge,
deployment, paid-model call, farm/production-database/provider effect,
permission/billing change or hardware operation occurred. Every recorded hold
and the original failed candidate remain preserved. No owner action is requested
until the technical admission transition is concrete and reviewable.

## 2026-09-18 21:51 SAST - Exact OMQ draft publication approved

Charl replied `I approve this.` to the exact candidate publication proposal:
push only `codex/oom-shutdown-visibility-20260918` at
`deff39235adf357ab19ac971804addff1171188e`, create one draft PR against main,
and run the existing hosted qualification. Canonical admission writes/callback,
merge, deployment, provider/farm/production-database effects, paid-model calls,
permission/billing changes and hardware remain held.

Fresh fetched main is still `e46743cb3e8d224b60d17eb5920acb113f613a52`;
source HEAD/tree are exact and clean. All 96 preserved PR heads match. The only
historical file overlap is PR #1154, unchanged head `7a7d5392...`, in
`telegram_gateway.py`; that separate Anton mission/PR is retained and neither
merged nor modified. The approved step is draft publication and qualification.
Publication preflight and exact approved body are retained under
`control-tower-artifacts/OMQ-20260813-03/desktop-20260918/publication/`.

## 2026-09-18 20:59 SAST - OMQ local successor qualified; publication held

Charl's approved local source/test repair and independent review are complete
under existing mission `OMQ-20260813-03`. Exact successor
`deff39235adf357ab19ac971804addff1171188e`, tree
`618d4d933b3d127498b65bbb45c53b9aac7e6073`, is retained on clean local branch
`codex/oom-shutdown-visibility-20260918`, one ahead/zero behind current main
`e46743cb3e8d224b60d17eb5920acb113f613a52`. The 23-file diff carries the
retained candidate plus the bounded shutdown-visibility repair. Writer evidence:
329 tests and 8 subtests passed. Independent exact-head review: complete diff
reviewed, 24 tests passed, no remaining local source blocker. This is PREPARED /
LOCAL SOURCE QUALIFICATION PASS, not release or business completion.

The five original findings, original candidate `86bcee270...` and its failed
review remain preserved. Corrected-language reviewer rerun retained its initial
false-negative assertion and did not change application source. All failed
attempts and inherited approvals/holds remain timestamped evidence.

Current handover and complete Check Receipt:
`control-tower-artifacts/OMQ-20260813-03/desktop-20260918/LOCAL_SUCCESSOR_QUALIFICATION_RECEIPT_20260918.md`.
Exact source/reviewer packets and the publication proposal are retained beside
it. Both bounded engineering workers are released-retain; Desktop remains sole
coordinating owner. Former coordinator must perform no dispatch/release work.

Fresh 20:51 read-only checks confirm clean main equal to origin/main, all 59
inherited worktrees preserved, 62 total after three local checkouts, all 24
quarantines present/unregistered, four preservation refs matching and the same
96 open PR heads. Production at 20:52 still serves exact
`3d321e4932cf205d03e759cc71a26edd411bc9d2`, Render live, identity complete.
Linda, C Camp, plan-hash, Anton and every recorded operational hold remain.

Next exact gate: owner approval to publish only this reviewed branch/head as a
draft PR for existing hosted qualification. The concrete proposed title/body
and boundary are in
`control-tower-artifacts/OMQ-20260813-03/desktop-20260918/successor/EXACT_CANDIDATE_PUBLICATION_PROPOSAL_20260918.md`.
No publication has occurred. Canonical admission writes/callback, merge,
deployment, provider/farm/production-database effects, paid-model calls,
permission/billing changes and hardware remain held. Missing admission must
remain visible; no prior receipt or PR #1336 release approval is reused.
Hosted/full PostgreSQL/browser qualification, trusted admission and later
genuine deployed-agent/terminal-independent acceptance remain open.

## 2026-09-18 20:27 SAST - Local shutdown-visibility repair approved

Charl replied `Approved, please continue` to the exact proposal for local
OMQ-20260813-03 source/test repair, local commits, synthetic tests and independent
review. This resolves the automatic approval review's earlier source-mutation
boundary; the rejection and original failed candidate remain preserved.

Resume sole source writer `/root/oom_shutdown_successor` in
`.codex-runtime/missions/OMQ-20260813-03/shutdown-successor`, branch
`codex/oom-shutdown-visibility-20260918`, clean at freshly fetched main
`e46743cb3e8d224b60d17eb5920acb113f613a52`. Scope is the five findings in
`control-tower-artifacts/OMQ-20260813-03/desktop-20260918/successor/SHUTDOWN_VISIBILITY_PROPOSED_REPAIR_20260918.md`.
The independent reviewer remains separate and will review the exact committed
successor. Control Tower owns only coordination documentation and decisions.

No publication, admission, PR, push, merge, deployment, provider/farm/database
write, paid-model probe, permission/billing change or hardware operation is
included. Original candidate, all 59 inherited worktrees and every existing
approval/hold/evidence boundary remain preserved. No further owner action is
needed for the approved local implementation and review.

## 2026-09-18 - Desktop ownership transferred after verification pass

Charl explicitly transferred sole coordination from
`01a08bfa-ca70-7a01-91df-fc3dd3922598` to
`CONTROL-TOWER-DESKTOP-SUCCESSOR-20260918` (Desktop task
`01a0b55a-6896-7643-b267-25cd4bcce5d7`) after the 19:25 SAST verification
pass at clean main `e46743cb3e8d224b60d17eb5920acb113f613a52`.
The former session must perform no further dispatch or release work.

The successor begins the handover's first concrete action in existing mission
`OMQ-20260813-03`: independent local qualification of missed-shutdown and
unverified-OFF visibility at retained candidate
`86bcee2709374353b7daf8d68bda839bac69337c`, tree
`8582052f2cab5bf1a7abc4d26c27ebb085c6e23b`. No application writer or runtime
release is assigned. Every prior approval, hold, failed review, working copy,
mission identity and evidence boundary remains intact.

Transfer receipt and current continuation route:
`control-tower-artifacts/REPOSITORY-CONSOLIDATION-20260918/DESKTOP_COORDINATING_OWNERSHIP_TRANSFER_20260918.md`.
This local documentation transition grants no candidate publication, merge,
deployment, provider, database/farm, permission, billing or hardware authority.

The exact retained head failed the independent synthetic shutdown-visibility
gate (8 tests: 4 pass, 4 fail). Overdue Active and late verified OFF can remain
silent; contained unverified OFF can read as safe/no action; a distinct failed
execution can be incorrectly deduplicated. Original head, tree, working copy,
passing historical suite and fresh failed review evidence remain preserved.

Next safe stage: one bounded local implementation writer on branch
`codex/oom-shutdown-visibility-20260918`, isolated worktree
`.codex-runtime/missions/OMQ-20260813-03/shutdown-successor`, from exact
current main. Carry the retained candidate's changes into that isolated branch
and repair execution-bound shutdown/deadline classification, presentation and
deduplication. No protected execution behavior or authority is expanded.
The independent reviewer retains original failed evidence and will assess the
exact successor after focused tests. Registry count becomes 62 solely because
of the three new local coordination/review/repair checkouts; inherited 59 remain
unchanged. Candidate publication and release remain held.

Source mutation was not performed: automatic approval review rejected the
dispatched `git cherry-pick --no-commit 86bcee2709374353b7daf8d68bda839bac69337c`
into the new clean successor worktree, interpreting the owner transfer as
qualification authority only and requiring explicit source-edit authority.
The worker was immediately limited to read-only diagnosis and a concrete
repair specification. No retry, alternate patch application, source edit or
commit is authorized until that approval boundary is resolved. The new repair
checkout remains at current main; original review is completed independently.

The corrected independent harness reproduces 4 failures / 4 passes; 19
supporting standard pytest tests pass. A separate real-adapter-shape case fails
3 subtests because controller output supplies `retrieved_at` while candidate
presentation expects `observed_at`. The complete five-finding review and exact
seven-file proposed repair are preserved under
`control-tower-artifacts/OMQ-20260813-03/desktop-20260918/`.
Current first-action receipt:
`control-tower-artifacts/OMQ-20260813-03/desktop-20260918/CONTROL_TOWER_FIRST_ACTION_RECEIPT_20260918.md`.
Both bounded workers have released; the source lane's exact wake is explicit
local source/test/commit approval. Runtime publication remains a later separate
gate. The original candidate, main and all inherited working copies are unchanged.
