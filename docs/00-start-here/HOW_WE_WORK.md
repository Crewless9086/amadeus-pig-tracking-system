...
| **Status** | Phase 8E breeding-board sorting/action loop is deployed and owner-verified. Phase 8F first read-only analytics slice is deployed and owner-verified. Phase 8F drill-in/data-quality slice is local and ready for deploy/browser acceptance. 9.7A lifecycle outcome audit is documented. 9.7B death/removal action is deployed-visible and real active-pig dry-run verified, but still awaiting a real write case. 9.7C slaughter pig-exit confirmation is available for open slaughter sales only. 9.7D dashboard outcome feedback is deployed/layout-refined. 9.7E closed-action cleanup is deployed-visible; local API inspection/reconciliation now fixes incomplete closed slaughter lifecycle links. 9.7F first backend newborn health action is local and real dry-run verified with Ecomectin as `Antiparasitic` and Panacur as `Deworming`. `/bulk-weights` remains open until the real farm weighing test passes. `/sales-dashboard` remains open until owner browser acceptance. |
| **Current focus** | Add computed newborn-health litter attention reason/dashboard digest support, then browser-check the `/litter/<litter_id>` newborn health preview/apply panel. |
| **Current last verified** | **2026-06-02** - Phase 9.7F newborn health backend dry-run against `LIT-2026-9E4A` planned 10 earmark updates, 10 `Antiparasitic` Ecomectin rows, and 10 `Deworming` Panacur rows without writing; focused tests passed. |
