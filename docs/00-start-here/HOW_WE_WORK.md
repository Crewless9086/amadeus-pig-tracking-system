...
| Field | Value |
|--------|--------|
| **Today's focus** | Phase 9.7G smart wean/tag attention timing |
| **Status** | Phase 8E breeding-board sorting/action loop is deployed and owner-verified. Phase 8F first read-only analytics slice is deployed and owner-verified. Phase 8F drill-in/data-quality slice is local and ready for deploy/browser acceptance. 9.7A lifecycle outcome audit is documented. 9.7B death/removal action is deployed-visible and real active-pig dry-run verified, but still awaiting a real write case. 9.7C slaughter pig-exit confirmation is available for open slaughter sales only. 9.7D dashboard outcome feedback is deployed/layout-refined. 9.7E closed-action cleanup is deployed-visible. 9.7F newborn health action is live-tested complete on `LIT-2026-9E4A`. 9.7G is local: estimated wean date uses 35 days, tag/wean attention starts 3 days before estimated wean date, early dashboard tag reminders are suppressed, and litter detail shows timing fields. `/bulk-weights` remains open until the real farm weighing test passes. `/sales-dashboard` remains open until owner browser acceptance. |
| **Current focus** | Deploy/browser-check 9.7G wean timing, then move to 9.7H fast pre-weaning death/stillborn handling with dead pig rows preserved for history. |
| **Current last verified** | **2026-06-04** - Phase 9.7G local verification passed: `LIT-2026-9E4A` estimated wean date `2026-06-22`, attention start `2026-06-19`, no dashboard tag attention today, route smoke passed, and full suite passed at 336 tests. |
