...
- **Explicit scope:** Phase 10.6 Oom Sakkie local kiosk/backend-as-brain work through `10.6Z`, plus Phase 10.7A-Z, 10.8A-P, and 10.9A-EE specialist manifest, advisory trace-review, access caveat hardening, kiosk review-advisor panel, advisor wording/proxy-test tightening, advisor trace-read consolidation, advisor SQL/test hardening, kiosk advisor-window/voice-loop counter polish, trace-driven router/power-answer tightening, capability-fallback precedence fix, bounded LLM fallback router, LLM fallback privacy/failure-mode hardening, LLM smoke harness, verified local LLM smoke, env-gated LLM answer composer, Usejarvis external-reference review, kiosk alive-state/provenance strip, stronger spoken answer voice, animated presence orb, capped context briefing composer, read-only operating brief tool, operating brief required-section fix, top voice controls, human-approved Learning Queue, explicit LLM Learning Analyst, trace-driven composer lane guard, deterministic Learning Build Brief packet, human-approved Implementation Queue, Approve For Build gate, persistent build request queue, build request event log, Forge Handoff packet, Forge Handoff persisted-ID hardening, Patch Proposal Gate, Patch Gate review nits, Deploy Approval Gate, Workbench simplification, Forge prompt copy button, read-only system work status tool, Workbench pipeline clarity, deploy-ready instructions, read-only Business Advisor seed, Workbench Next Action card, Business quick action, Work Status honesty fix, Business Advisor context upgrade, role-specific spoken composer rules, internal-only Business Offer Outline, agent runtime/readiness/authority/review rails, Sentinel single-shot advisory runner and first supervised smoke, Owner Cockpit UI, learning influence proposal rail/status/workbench/browser gates, cockpit decision feedback, exact accepted-result proposal preparation, owner review packet scope evidence, the `prepare Claude review` chat gate, live-PG closure for the learning-influence `from-result` rail, recorded-CI evidence policy, learning-consumption threat-model readiness, explicit provenance/blast-radius controls, read-only consumption audit-rail blueprint, append-only consumption audit rail implementation, pre-consumer static/design gate, hardened allow-consumed static guard, source-backed allow-consumed caller evidence, scanner resilience hardening, read-only consumer design agreement, consumed-once atomicity wording, owner-approved first review-note-only learning consumer, call-site-granular allow-consumed guard hardening, browser voice-capture fix, supervised live consumer smoke evidence, backend push-to-talk STT fallback, read-only Telegram gateway path, Telegram LLM-egress/token hardening, Telegram public-exposure hardening, Telegram exposure preflight, private Telegram relay smoke helper, smoke URL guard, inactive n8n backend read-only relay contract, relay transport guard, local relay import preflight, Workbench learning-backlog clarity, owner-approved GateKeeper backend-relay wiring plan, 2.0B manual execution helper, n8n live-state inspector, Render gateway smoke hardening, Render gateway live-ready smoke, n8n Cloud variable compatibility for 2.0B, 2.0B request-ready HTTP gate, 2.0B base URL normalizer/diagnostic, owner-approved backend-owned direct Telegram for Oom Sakkie, direct Telegram parity/menu/formatting, default-off proactive daily brief sender, compact structured Telegram layouts, the logged agent/Sam boundary for future specialist activation, the owner-review sales offer brief lane, daily-brief dry-run preview, owner-review customer draft rail, env-gated Ledger sales agent advisory lane, append-only sales campaign rail, sales campaign owner approval plus internal outreach draft queue, and local Ledger Sales Workbench plus append-only Sam/Chatwoot send-design request rail.
- **Latest additions in this review packet:** Phase 10.9DB-DT adds an env-gated backend STT fallback for the existing owner-clicked `Talk` / `Talk & Ask` controls, a token-gated read-only Telegram gateway endpoint, the follow-up hardening requested by Claude, a protected read-only exposure preflight, a local private relay smoke helper, a URL guard for that helper, an inactive callable n8n backend relay contract for GateKeeper review, a matching n8n base-URL transport guard, a local import preflight validator, kiosk Workbench clarity for learning/dry-run backlog, an owner-approved manual GateKeeper backend-relay wiring plan, a 2.0B manual execution helper, a read-only n8n live-state inspector, Render gateway smoke hardening, Render gateway live-ready smoke, n8n Cloud variable compatibility for `2.0B`, the request-ready HTTP gate, and a base URL normalizer/diagnostic. The STT fallback records only after an owner click, posts one short browser audio blob to the protected Flask route, stores no audio, leaves always-on mic/wake word off, and keeps writes/dispatch/runtime changes false; owner note: captured audio is sent to the configured OpenAI audio transcription endpoint when this fallback is enabled. The Telegram gateway accepts a Telegram-update-shaped payload only when `OOM_SAKKIE_TELEGRAM_GATEWAY_ENABLED=1`, `OOM_SAKKIE_TELEGRAM_GATEWAY_TOKEN` is at least 32 characters, and `OOM_SAKKIE_TELEGRAM_ALLOWED_USER_IDS` is configured; token comparison uses `hmac.compare_digest`, repeated bad tokens trigger a fail-closed in-process auth lockout, and the `telegram_read_only` channel is deterministic-only with a true no-HTTP-egress regression test so it cannot trigger outbound LLM calls even if those env flags are enabled. Protected `GET /api/oom-sakkie/channels/telegram/exposure-preflight` separates automated private-test readiness from public-exposure readiness, records the in-process/global rate-limit caveat, and requires explicit TLS plus rate-limit-model confirmations before reporting public ready. `scripts/oom_sakkie_telegram_private_relay_smoke.py` checks preflight and one gateway reply payload without sending Telegram, and refuses non-local plain-HTTP smoke URLs before sending the token. `docs/04-n8n/workflows/2.0B - Oom Sakkie Backend Read-Only Relay/` adds a callable `Execute Workflow Trigger` template only: no Telegram Trigger, no Telegram send node, no hardcoded token/chat ID, and no second normal-message owner. The relay validates backend no-authority flags, refuses remote plain-HTTP base URLs before using the bearer token, and returns only a guarded `send_allowed` / `chat_id` / `telegram_text` payload for GateKeeper's existing send path. `scripts/oom_sakkie_n8n_relay_contract_check.py` validates the committed workflow/README locally without calling n8n, Telegram, Flask, OpenAI, Google Sheets, or Supabase. The Workbench labels closed dry-runs, accepted evidence, and reviewed learning proposals as append-only audit backlog, and dry-run requests with recorded results no longer appear as active handoff work. `BACKEND_RELAY_WIRING_PLAN.md` says to import `2.0B` inactive, edit GateKeeper manually through n8n UI after backup, keep GateKeeper as the only Telegram Trigger owner and only reply sender, validate backend no-authority flags before sending exactly one owner reply, and preserve rollback. `scripts/oom_sakkie_n8n_relay_manual_test.py --payload` prints a token-free manual n8n payload, and `--validate-output` blocks copied 2.0B outputs unless caller-handled reply transport is present and all authority flags remain false. `scripts/oom_sakkie_n8n_live_state_check.py` verified live n8n now has GateKeeper targeting `2.0B`, while `2.0B` still has no Telegram trigger/send node. `scripts/oom_sakkie_telegram_gateway_smoke.py` now reuses the first configured allowed Telegram user by default and refuses remote plain-HTTP URLs before sending the bearer token; after the owner enabled the Render gateway env, running it against `https://amadeus-pig-tracking-system.onrender.com/api/oom-sakkie/channels/telegram/message` returned HTTP 200 `answered`, tool `farm_attention_summary`, and all send/LLM-egress/write/dispatch authority flags false. The live n8n error `access to env vars denied` was traced to `$env` reads in `Code - Build Backend Gateway Request`; the workflow export now uses n8n Variables through `$vars`, the HTTP node reads the bearer token directly from `$vars`, and `gateway_token` is no longer emitted into item JSON. The follow-up `URL parameter must be a string, got undefined` error was traced to setup failure continuing into the HTTP node; `IF - Gateway Request Ready` now routes setup failures directly to the validator/failure output. The Build node now strips common wrapping quotes/invisible paste characters, extracts a pasted full URL when possible, and returns a safe `base_url_diagnostic` if validation still fails. This does not add another learning consumer, apply learning, produce an applyable prompt/route/runtime diff, mutate prompts/routes/runtime/tools/farm data, create public/customer output directly outside GateKeeper's existing owner chat, deploy, control hardware, or perform financial actions.

- **Current slice for this review:** Phase 10.9EE adds the local Ledger Sales Workbench and append-only Sam/Chatwoot send-design request rail. It makes sales campaign/draft/send-design state visible in `/oom-sakkie`, adds review-gated send-design routes and `sales_send_design_status`, applies the updated live migration, and records one internal live send-design smoke. It still has no customer-send, Sam/Chatwoot/n8n/WhatsApp call, quote/order/reservation, stock change, dispatch, runtime/prompt change, farm write, public output, physical control, or financial action consumer.

- **10.9DU direct Telegram addition:** Owner approved backend-owned direct Telegram for Oom Sakkie. New files/functions are `modules/oom_sakkie/telegram_direct.py`, `POST /api/oom-sakkie/channels/telegram/direct-webhook`, `scripts/oom_sakkie_telegram_direct_webhook.py`, and `scripts/oom_sakkie_telegram_direct_smoke.py`. The direct path is default-off, requires `OOM_SAKKIE_TELEGRAM_DIRECT_ENABLED=1`, `OOM_SAKKIE_TELEGRAM_DIRECT_SEND_ENABLED=1`, `OOM_SAKKIE_TELEGRAM_BOT_TOKEN`, a 32+ character `OOM_SAKKIE_TELEGRAM_WEBHOOK_SECRET` supplied by Telegram as `X-Telegram-Bot-Api-Secret-Token`, and `OOM_SAKKIE_TELEGRAM_ALLOWED_USER_IDS`. It handles allowed owner updates with `channel=telegram_read_only`, then sends the owner reply through Telegram `sendMessage`. It intentionally removes n8n from the Oom Sakkie owner chat path once configured, while keeping `can_trigger_outbound_llm=false`, `writes=false`, `dispatch_enabled=false`, `changes_runtime_now=false`, `changes_prompt_now=false`, `physical_controls_enabled=false`, and `customer_public_output_enabled=false`.
- **10.9DV direct Telegram activation layer:** Added review-gated `GET /api/oom-sakkie/channels/telegram/direct-parity`, direct Telegram shortcut commands (`/start`, `/help`, `/brief`, `/attention`, `/gates`, `/agents`, `/approvals`, `/progress`, `/learning`), formatted Telegram replies with tool/safety footer, and `scripts/oom_sakkie_telegram_daily_brief.py` for explicit daily-brief sends. The daily brief is default-off behind `OOM_SAKKIE_TELEGRAM_PROACTIVE_ENABLED=1` and `OOM_SAKKIE_TELEGRAM_DAILY_BRIEF_ENABLED=1`, has no Flask background loop, sends only to `OOM_SAKKIE_TELEGRAM_ALLOWED_USER_IDS`, uses `telegram_read_only`, and keeps outbound LLM, farm/control writes, dispatch, prompt/runtime changes, physical controls, customer/public broadcast expansion, and financial action false.
- **10.9DW live Telegram formatting polish:** After the owner's live Telegram shortcut test, direct Telegram now formats daily brief, safety gates, and agent command center using compact structured layouts. `handle_message()` returns bounded `tool_context` from each tool's existing `llm_context`; Telegram formatting uses that structure instead of parsing prose. Generic/free-text answers continue through the standard formatter. This is presentation-only and keeps the same owner-only/no-LLM/no-write/no-dispatch/no-runtime-change envelope.
- **10.9DX agent/Sam direction log:** Captures the intended next architecture before live specialist work: Oom Sakkie/Jarvis stays the owner orchestrator and backend command center; specialist agents start as backend draft/review-only lanes; Sam keeps Chatwoot/n8n plus the order steward as customer-facing transport/order plumbing until a separate sales-agent authority review. The first useful backend Sales/Sam step should be owner-reviewable offer/reply/quote recommendations only, not customer send or order mutation. Irrigation follows the same ladder: read-only recommendations, then owner-approved action requests, then physical-control authority only after a dedicated safety review. This is documentation/review-scope only and adds no runtime authority.
- **10.9DY owner sales offer brief:** Adds `sales_offer_brief` and direct Telegram `/offer`. It uses existing read-only sales dashboard and meat planning context through `business_growth_brief` to produce a Ledger-shaped owner-review brief with target, stock basis, owner checks, and approval question. It is explicitly not customer copy and does not send Telegram/customer messages, post publicly, create quotes, reserve/change stock, update orders, run specialist LLM/tools, dispatch, write farm/control/customer data, or change runtime.
- **10.9DZ daily brief dry-run preview:** Adds `scripts/oom_sakkie_telegram_daily_brief.py --dry-run` and backend preview helper. The preview builds the same direct Telegram daily brief text, reports allowed-recipient count, and prints no-authority flags without calling Telegram `sendMessage`. Normal script send behavior remains gated by the existing direct/proactive/daily env flags. No scheduler, background loop, hidden send, write, dispatch, runtime change, physical control, customer output, or financial action was added.
- **10.9EA owner-review customer draft rail:** Adds `sales_customer_draft` and direct Telegram `/draft`. It turns the current sales offer brief into short buyer-interest copy with placeholders and owner checks. The tool is `DRAFT_ONLY` and counted separately from write/authority tools. It calls no Chatwoot, n8n, WhatsApp, Telegram customer chat, or public channel; it creates no quote, order, reservation, price change, stock change, dispatch, specialist LLM/tool execution, runtime change, farm/control write, or financial action.
- **10.9EB Ledger sales agent advisory lane:** Adds `modules/oom_sakkie/ledger_agent.py`, `ledger_sales_agent`, deterministic routing, runtime policy fields, and direct Telegram `/ledger`. This is the first owner-approved smart business lane: only when `OOM_SAKKIE_LEDGER_AGENT_ENABLED=1`, `OPENAI_API_KEY`, and `OOM_SAKKIE_LLM_ROUTER_MODEL` are configured, and only when the explicit owner `/ledger` command path sets `allow_specialist_llm=true`, it sends capped owner sales/meat context to the OpenAI-compatible chat-completions endpoint and returns owner-review strategy, buyer-copy improvements, owner questions, risks, and a next action. Disabled/misconfigured/non-authorized request mode makes no network call. Direct Telegram policy discloses that explicit `/ledger` can trigger outbound LLM when the Ledger env gate is enabled; generic Telegram remains deterministic. The parser rejects unsafe LLM action claims. It remains `DRAFT_ONLY` and cannot send customer messages, call Chatwoot/n8n/WhatsApp, create quotes/orders/reservations, change stock, dispatch agents, change prompts/runtime, write farm/customer/control data, or perform financial/physical actions.
- **10.9EC sales campaign rail:** Adds migration `202606140001_create_oom_sakkie_sales_campaigns.sql`, `modules/oom_sakkie/sales_campaign_store.py`, review-gated `/sales-campaigns` routes, read-only `sales_campaign_status`, Telegram `/campaigns`, and Ledger campaign recording after a successful `/ledger` LLM advisory run. Campaigns persist opportunity/draft/questions/risks/next-action state for Jarvis owner review. DB CHECK constraints and app flags force customer send, Chatwoot/n8n calls, quote/order creation, stock changes, dispatch, runtime/prompt changes, farm-data writes, public output, and physical controls false. `approved_for_customer_outreach` is an append-only event label only; no consumer sends anything in this slice.
- **10.9ED campaign approval and outreach draft queue:** Extends the same sales migration with append-only `oom_sakkie_sales_outreach_drafts`, adds review-gated `/sales-outreach-drafts` routes, read-only `sales_outreach_draft_queue`, Telegram `/drafts`, and owner-only Telegram `/approve_campaign`. The approval command records one append-only `approved_for_customer_outreach` event on the first waiting campaign and queues one internal customer outreach draft for owner review. It sends no customer message, calls no Chatwoot/n8n/WhatsApp, creates no quote/order/reservation, changes no stock, dispatches no agent, changes no runtime/prompt, writes no farm data, and performs no public/physical/financial action.
- **10.9EE Ledger Sales Workbench and send-design rail:** Extends the same sales migration with append-only `oom_sakkie_sales_send_design_requests`, adds review-gated `GET /sales-send-design-requests`, review-gated `POST /sales-outreach-drafts/<draft_id>/send-design-requests`, read-only `sales_send_design_status`, deterministic routing for send-design queue questions, and a local `/oom-sakkie` Ledger Sales Workbench. The workbench shows campaigns, outreach drafts, and Sam/Chatwoot send-design requests; owner-clicked web actions can approve the first campaign into an outreach draft and prepare a send-design request. Startup still performs no hidden POSTs, and the create endpoint is intentionally POST-only. The live DB migration was applied and one internal send-design request was recorded from the existing draft (`OSK-SALES-SEND-DESIGN-BCD9E5D9C2DD80C9`) with all authority flags false. There is still no customer-send, Sam/Chatwoot/n8n/WhatsApp call, quote/order/reservation, stock change, dispatch, runtime/prompt change, farm write, public output, physical control, or financial action consumer.

Out of scope unless explicitly asked:

- non-owner/public Telegram expansion beyond the owner-only direct Oom Sakkie bot
- write tools
- physical controls
- backend TTS vendors
- always-on mic
- wake word
- ungated specialist LLM delegation, specialist loops, or specialist tool execution beyond the explicit env-gated Ledger owner-review advisory lane
- Agent Factory
...
- `supabase/migrations/202606140001_create_oom_sakkie_sales_campaigns.sql`
- `docs/01-architecture/OOM_SAKKIE_VOICE_OPERATING_AGENT_PRD.md`
- `docs/01-architecture/OOM_SAKKIE_AGENT_ROSTER.md`
...
- 10.9DN 2.0B manual execution helper: `.\venv\Scripts\python.exe scripts\oom_sakkie_n8n_relay_manual_test.py --payload` -> token-free payload; `.\venv\Scripts\python.exe scripts\oom_sakkie_n8n_relay_contract_check.py` -> `relay_contract_status: ok`; `.\venv\Scripts\python.exe -m unittest tests.test_workflow_contracts` -> 24 OK.
- 10.9DO live n8n state inspector: `.\venv\Scripts\python.exe scripts\oom_sakkie_n8n_live_state_check.py` -> `n8n_live_state_status: ok`; GateKeeper active, `2.0B` active, GateKeeper target `2.0B_backend_relay`, backend relay has no Telegram Trigger and no Telegram send node.
...
- 10.9DQ Render gateway live-ready smoke after owner enabled Render env: `.\venv\Scripts\python.exe scripts\oom_sakkie_telegram_gateway_smoke.py` with `OOM_SAKKIE_TELEGRAM_GATEWAY_SMOKE_URL=https://amadeus-pig-tracking-system.onrender.com/api/oom-sakkie/channels/telegram/message` -> HTTP 200 `answered`, tool `farm_attention_summary`, answer `No current farm attention items are showing.`, `sends_telegram = False`, `can_trigger_outbound_llm = False`, `writes = False`, `records_audit_trace = True`, `dispatch_enabled = False`.
- 10.9DQ live n8n state check: `.\venv\Scripts\python.exe scripts\oom_sakkie_n8n_live_state_check.py` -> `n8n_live_state_status: ok`, GateKeeper active, `2.0B` active, GateKeeper target `2.0B_backend_relay`, backend relay has no Telegram Trigger and no Telegram send node.
- 10.9DR relay contract check: `.\venv\Scripts\python.exe scripts\oom_sakkie_n8n_relay_contract_check.py` -> `relay_contract_status: ok`; workflow has no `$env`, uses `$vars`, and does not emit `gateway_token` into item JSON.
- 10.9DR workflow contracts: `.\venv\Scripts\python.exe -m unittest tests.test_workflow_contracts` -> 25 OK.
- 10.9DR frontend route contracts: `.\venv\Scripts\python.exe -m unittest tests.test_frontend_route_contracts` -> 29 OK.
- 10.9DS relay contract check: `.\venv\Scripts\python.exe scripts\oom_sakkie_n8n_relay_contract_check.py` -> `relay_contract_status: ok`; workflow includes `IF - Gateway Request Ready` between Build and HTTP.
- 10.9DS workflow contracts: `.\venv\Scripts\python.exe -m unittest tests.test_workflow_contracts` -> 25 OK.
- 10.9DS frontend route contracts: `.\venv\Scripts\python.exe -m unittest tests.test_frontend_route_contracts` -> 29 OK.
- 10.9DT relay contract check: `.\venv\Scripts\python.exe scripts\oom_sakkie_n8n_relay_contract_check.py` -> `relay_contract_status: ok`; workflow includes `normalizeBaseUrl`, `cleanVariable`, and safe `base_url_diagnostic`.
- 10.9DT workflow contracts: `.\venv\Scripts\python.exe -m unittest tests.test_workflow_contracts` -> 25 OK.
- 10.9DT frontend route contracts: `.\venv\Scripts\python.exe -m unittest tests.test_frontend_route_contracts` -> 29 OK.
- 10.9DU direct Telegram compile: `.\venv\Scripts\python.exe -m py_compile modules\oom_sakkie\telegram_direct.py scripts\oom_sakkie_telegram_direct_webhook.py scripts\oom_sakkie_telegram_direct_smoke.py` -> OK.
- 10.9DU focused audit suite: `.\venv\Scripts\python.exe -m unittest tests.test_oom_sakkie_service tests.test_oom_sakkie_routes tests.test_frontend_route_contracts` -> 370 OK.
- 10.9DV direct Telegram activation compile: `.\venv\Scripts\python.exe -m py_compile modules\oom_sakkie\telegram_direct.py scripts\oom_sakkie_telegram_direct_webhook.py scripts\oom_sakkie_telegram_direct_smoke.py scripts\oom_sakkie_telegram_daily_brief.py` -> OK.
- 10.9DV focused audit suite: `.\venv\Scripts\python.exe -m unittest tests.test_oom_sakkie_service tests.test_oom_sakkie_routes tests.test_frontend_route_contracts` -> 378 OK, 7 skipped.
- 10.9DV workflow contracts: `.\venv\Scripts\python.exe -m unittest tests.test_workflow_contracts` -> 25 OK.
- 10.9DV JavaScript syntax: `node --check static/js/oomSakkie.js` -> OK.
- 10.9DV browser behavior smoke: `node tests/oom_sakkie_browser_behavior_smoke.js` -> passed.
- 10.9DW focused audit suite: `.\venv\Scripts\python.exe -m unittest tests.test_oom_sakkie_service tests.test_oom_sakkie_routes tests.test_frontend_route_contracts` -> 379 OK, 7 skipped.
- 10.9DW workflow contracts: `.\venv\Scripts\python.exe -m unittest tests.test_workflow_contracts` -> 25 OK.
...
- 10.9DY focused audit suite: `.\venv\Scripts\python.exe -m unittest tests.test_oom_sakkie_service tests.test_oom_sakkie_routes tests.test_frontend_route_contracts` -> 382 OK, 7 skipped.
- 10.9DY workflow contracts: `.\venv\Scripts\python.exe -m unittest tests.test_workflow_contracts` -> 25 OK.
- 10.9DY JavaScript syntax: `node --check static/js/oomSakkie.js` -> OK.
- 10.9DY browser behavior smoke: `node tests/oom_sakkie_browser_behavior_smoke.js` -> passed.
- 10.9DZ focused audit suite: `.\venv\Scripts\python.exe -m unittest tests.test_oom_sakkie_service tests.test_oom_sakkie_routes tests.test_frontend_route_contracts` -> 383 OK, 7 skipped.
- 10.9DZ script/module compile: `.\venv\Scripts\python.exe -m py_compile scripts\oom_sakkie_telegram_daily_brief.py modules\oom_sakkie\telegram_direct.py modules\oom_sakkie\tools.py` -> OK.
- 10.9DZ workflow contracts: `.\venv\Scripts\python.exe -m unittest tests.test_workflow_contracts` -> 25 OK.
- 10.9DZ JavaScript syntax: `node --check static/js/oomSakkie.js` -> OK.
- 10.9DZ browser behavior smoke: `node tests/oom_sakkie_browser_behavior_smoke.js` -> passed.
- 10.9EA focused audit suite: `.\venv\Scripts\python.exe -m unittest tests.test_oom_sakkie_service tests.test_oom_sakkie_routes tests.test_frontend_route_contracts` -> 386 OK, 7 skipped.
- 10.9EA module compile: `.\venv\Scripts\python.exe -m py_compile modules\oom_sakkie\tools.py modules\oom_sakkie\telegram_direct.py modules\oom_sakkie\service.py modules\oom_sakkie\policy.py` -> OK.
- 10.9EA workflow contracts: `.\venv\Scripts\python.exe -m unittest tests.test_workflow_contracts` -> 25 OK.
- 10.9EA JavaScript syntax: `node --check static/js/oomSakkie.js` -> OK.
...
- 10.9ED focused audit suite: `.\venv\Scripts\python.exe -m unittest tests.test_oom_sakkie_service tests.test_oom_sakkie_routes tests.test_frontend_route_contracts` -> 410 OK.
- 10.9ED module compile: `.\venv\Scripts\python.exe -m py_compile modules\oom_sakkie\sales_campaign_store.py modules\oom_sakkie\tools.py modules\oom_sakkie\routes.py modules\oom_sakkie\service.py modules\oom_sakkie\telegram_direct.py modules\oom_sakkie\agent_runtime.py` -> OK.
...
- 10.9EE focused audit suite: `.\venv\Scripts\python.exe -m unittest tests.test_oom_sakkie_service tests.test_oom_sakkie_routes tests.test_frontend_route_contracts` -> 417 OK.
- 10.9EE module compile: `.\venv\Scripts\python.exe -m py_compile modules\oom_sakkie\sales_campaign_store.py modules\oom_sakkie\tools.py modules\oom_sakkie\routes.py modules\oom_sakkie\service.py modules\oom_sakkie\agent_runtime.py` -> OK.
...
165. **2.0B Manual Execution Helper:** Does Phase 10.9DN give the owner a safe pre-wiring manual n8n test by printing a token-free payload and validating copied 2.0B output for caller-handled reply transport plus all-false authority flags, without calling n8n, Telegram, Flask, OpenAI, Google Sheets, Supabase, or adding send/cutover/write/dispatch/runtime authority?
166. **Reverse proxy deployment rule:** Does the PRD now state strongly enough that same-host reverse proxying in front of review routes is forbidden until trusted proxy handling/auth is deliberately configured?
167. **Ledger Sales Workbench / Send-Design Rail:** Does Phase 10.9EE correctly move Ledger sales campaign/draft/send-design visibility into the local `/oom-sakkie` workbench, keep startup free of hidden POSTs, require explicit owner clicks for campaign approval and send-design request creation, enforce append-only/no-authority DB checks for `oom_sakkie_sales_send_design_requests`, and still avoid any Sam/Chatwoot/n8n/WhatsApp/customer send, quote/order/reservation, stock change, dispatch, runtime/prompt, farm-write, public-output, physical-control, or financial-action consumer?
168. **Tests:** What missing tests or browser checks should happen before this is considered daily-use ready?
