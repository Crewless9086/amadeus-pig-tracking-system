...
- Owner Review Packet focused verification:
  ...
  - Full local unittest suite: `python -m unittest` -> 588 tests OK
- Review Shortcuts verification pending in this Codex pass:
...
118. **Reverse proxy deployment rule:** Does the PRD now state strongly enough that same-host reverse proxying in front of review routes is forbidden until trusted proxy handling/auth is deliberately configured?
119. **Tests:** What missing tests or browser checks should happen before this is considered daily-use ready?
