...
- Consumed-Once Atomicity Wording that names the DB partial unique index as the future consumer's race guard without implementing that consumer
...
157. **Reverse proxy deployment rule:** Does the PRD now state strongly enough that same-host reverse proxying in front of review routes is forbidden until trusted proxy handling/auth is deliberately configured?
158. **Tests:** What missing tests or browser checks should happen before this is considered daily-use ready?
