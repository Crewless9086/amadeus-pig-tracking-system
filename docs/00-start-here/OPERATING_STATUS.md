...
Later fault addendum: **2026-07-25 15:04 UTC** for the SAM outbound
...
| --- | --- | --- |
| **SAM outbound delivery truth is accepted-unverified.** AUTO_GENERAL canary conversation `2013` created outgoing Chatwoot message `759446597`; the reply was conversationally correct but remained `status=sent` without provider delivered/read confirmation. Read-only evidence for conversations `2009`, `2008`, `2007`, `875`, `2005`, and `2006` showed the same accepted-but-unconfirmed outcome. The application path treated HTTP 2xx as `chatwoot_send_confirmed`, retained application-generated source identity where provider identity was expected, and did not expose a reliably recoverable exact delivery claim/outcome through the conversation. | State is `accepted_unverified`, not delivered and not failed. No automatic retry occurred. AUTO_GENERAL canary was disabled. Do not expose customer content or provider identity, and do not let Telegram/card cleanup claim customer completion. | Implement the shared [`Outbound Delivery Truth Standard`](../09-vault-brain/07-standards/OUTBOUND_DELIVERY_TRUTH_STANDARD.md): exact claim-before-send, outgoing-message and separate provider identity, append-only reconciliation, accepted/ambiguous no-retry, dashboard/daily accepted-unverified surfacing, and one controlled real provider delivered/read proof before canary graduation. |
