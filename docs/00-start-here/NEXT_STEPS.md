...
- P0 Staged Batch Auto-Process: active branch `p0-staged-batch-auto-process`; existing staged batch `2241aeab-4f40-4797-882d-1588a17abbd0` has 42 processable rows and must resume with Continue Upload instead of restaging or showing no actionable rows.
...
- Do not ask the owner to manually re-enter/test 71 or 73 rows again until automated existing-staged-batch recovery, Continue Upload, auto-processing, import/download, failure/retry, duplicate/already-recorded, and blank/no-change wording tests pass and the fix is deployed.
