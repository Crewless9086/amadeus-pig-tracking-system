...
0. **BMQ-20260813-02 - repair and prove the deployed private album intake.** The append-only schema and private bucket exist. Four 15 August provider photos remain recoverable, but Render omitted the required media policy variables and the shared gateway admitted generic context before typed BEACON media. Release the bounded precedence/context/receipt correction, activate every fail-closed policy input, recover the immutable four-photo album through deployed intake, then prove one fresh album produces one ordered contact sheet and one exactly-once receipt. Library acceptance, public-use, campaign approval and publication remain separate. Videos and historical folder import remain later bounded work.
