...
- GS-MIG-8 current cutover scope: order list/detail/search reads, document metadata reads/writes, document settings reads, daily report status-log reads, guarded order create/update/line/reservation/lifecycle writes, and order intake update/reset now prefer Supabase with Sheets fallback when unavailable.
...
- Formula-specific newborn-health attention replacement, mating/breeding mutation routes, sales transaction lifecycle pig updates, and any remaining legacy setup/import scripts remain outside the current safe cutover until explicit Supabase services and tests exist.
