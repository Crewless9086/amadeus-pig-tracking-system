...
| Phase 11: Pork Sales Business Module | Meat Sales private pilot rails are built through Sam intake, price book, Butcher match, carcass ops, deposit gate, instruction drafts/sends, fulfilment, driver route, journey sends, packed-weight reconciliation, final balance, delivery-release gate, Chatwoot sales hygiene, Sam Meat pressure hardening, structured buyer-preference capture, EFT-only VAT-inclusive meat document packet/PDF generation, gated Chatwoot estimated-quote attachment send, stale WhatsApp-window/template-required protection, and a token-gated Chatwoot/WhatsApp delivery-status consumer | Next: create and approve the pilot WhatsApp templates in `docs/08-business-modules/MEAT_SALES_WHATSAPP_TEMPLATES.md`, configure `MEAT_SALES_QUOTE_READY_TEMPLATE_NAME`, then live-smoke a fresh inbound WhatsApp lead where Sam sends the quote PDF inside the open window and delivery/read/failure callbacks update the lead automatically. |
...
### Phase 11G Complete - Instruction Approval, Send, And Exception Review
...
### Phase 11H Complete - Meat Fulfilment Timeline And Customer Journey Gates

Owner direction on 2026-06-16: real meat sales need to handle waiting half-carcass customers, WhatsApp 24-hour windows/templates, abattoir confirmation, butcher confirmation, delivery scheduling, driver assignment, and story-led customer updates without spamming customers.

Required outcome:

- Append-only fulfilment events log customer waiting state, WhatsApp service-window/template state, abattoir requested/confirmed, butcher requested/confirmed, delivery address capture, delivery scheduling, driver assignment, delivery progress, delivery completion/failure, and exception review.
- A half-carcass reservation remains `waiting_for_second_half` until another buyer completes the carcass or the owner chooses another approved path.
- The customer journey planner identifies whether a normal service-window reply is allowed or a template is required when the 24-hour WhatsApp window is closed.
- Abattoir and butcher slot confirmations must be logged before delivery timing is promised.
- Delivery address, town, route/window, and driver assignment are captured as separate fulfilment events.
- Farm App `/sales/meat-leads` exposes a fulfilment timeline panel to record and review these events.
- This build plans/logs customer journey updates only; no customer notification, driver notification, stock change, slaughter completion, or public output is sent automatically.
...
- `/sales/meat-driver` shows a focused delivery route page filtered by driver and date.
...
### Phase 11M Complete - Sales Stress-Test Pack
...
- `modules/sales/sam_meat_stress.py` defines 40 local stress scenarios.
- `scripts/sam_meat_stress_test.py` runs the pack without writing to the live database or Chatwoot.
- `tests/test_sam_meat_stress.py` guards coverage and launch-blocking behavior.
- First run report: `docs/08-business-modules/MEAT_SALES_STRESS_TEST_REPORT.md`.

Verified:

- Stress pack passed 40/40 launch-blocking assertions.
...
- 2026-06-15 update: this future function is now the active Herdmaster purpose-review phase. Build `/purpose-review` as the human approval table for weaned piglets with blank/`Unknown` purpose, with approve selected/all-visible, override, defer, and recheck paths. Approved writes are backend-owned and limited to `PIG_MASTER.Purpose`, `Updated_At`, and an appended `General_Notes` audit note. It must not write orders, sales, slaughter, meat allocation, customer channels, Telegram, Meta, Chatwoot, WhatsApp, or Supabase.
- 2026-06-15 local build note: `/purpose-review`, purpose recheck, owner approval apply, mark-weaned latest-weight carryover, `PIG_MASTER` wean-field enrichment, 14-day post-wean attention delay, and action-specific navigation are in place locally. Testing still needs a real future litter cycle: mark as weaned with latest weights, wait for/record a post-wean weight, confirm dashboard routing, review Herdmaster suggestions, and approve/override at least one row before marking this function farm-proven.
...
- GitHub Actions after commit `049f37c`: `Oom Sakkie Browser Behavior` run `27354775052` success; `Oom Sakkie Audit Rails` run `27354774018` success.
- Live-gated focused audit suite with `.env` loaded: `326 OK`.
- Browser behavior smoke passed.
- Full local unittest suite: `656 OK`.
...
- GitHub Actions after commit `d69ffdd`: `Oom Sakkie Browser Behavior` run `27361204620` success; `Oom Sakkie Audit Rails` run `27361204040` success.
...
- Move from local gateway smoke to an import-ready n8n relay contract without creating a second Telegram listener or send path.
- Preserve GateKeeper as the normal Telegram message owner.
- Keep Flask as the Oom Sakkie brain and n8n as a thin adapter.

What changed:

- Added `docs/04-n8n/workflows/2.0B - Oom Sakkie Backend Read-Only Relay/README.md`.
- Added inactive `workflow.json` with:
  - `Execute Workflow Trigger`,
  - message normalization from GateKeeper-style fields,
  - env-only backend base URL and gateway token,
  - HTTP POST to `/api/oom-sakkie/channels/telegram/message`,
  - caller-send reply validation.
- Updated `docs/04-n8n/WORKFLOW_MAP.md` to document `2.0B` as a backend relay contract.
- Added a frontend route contract test that parses the workflow JSON and asserts:
  - `active = false`,
  - no `telegramTrigger`,
  - no Telegram send node,
  - no hardcoded smoke token or Charl chat ID,
  - gateway endpoint and env names are present,
  - no-authority flags are validated.
...
- No prompt/route/runtime/tool/farm-data mutation.
- No write authority, dispatch, public output, physical control, or financial action.
