...
- GS-MIG-8 current cutover scope: order list/detail/search reads, document metadata reads/writes, document settings reads, daily report status-log reads, guarded order create/update/line/reservation/lifecycle writes, sales transaction slaughter-exit pig updates, order intake update/reset, mating/breeding mutation routes, and direct farm master/weight/treatment/movement writes now prefer Supabase with Sheets fallback when unavailable.
