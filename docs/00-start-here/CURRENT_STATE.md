...
- Phase 9.1C is implemented locally: litter detail now shows attention metadata and provides a backend-owned `Mark as Weaned` action that updates `LITTERS` and linked active/on-farm `PIG_MASTER` piglets with the chosen wean date/count. Purpose/classification review remains a later slice. A 2026-05-30 audit confirmed `LIT-2026-OTY0` and `LIT-2026-0LBF` are record-reconciliation attention items, while `LIT-2026-8A0F` needs piglet tag numbers; the local detail page now shows reason-specific actions and does not offer `Mark as Weaned` for those mismatch/tagging cases.
