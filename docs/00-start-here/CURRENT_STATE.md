...
- Phase 9.7I smart return navigation is local: litter piglet table links now carry `return_to=/litter/<litter_id>` and `return_label=Back to Litter`, and pig detail uses that safe internal return context to replace the header back link when opened from a litter. Full local unittest suite passed at 340 tests on 2026-06-04. Next closure step is deploy/browser-check from litter table to pig detail and back.
