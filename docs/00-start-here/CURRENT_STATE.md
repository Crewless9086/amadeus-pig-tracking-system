...
- Farm attention digest workflow is active in n8n with manual Telegram delivery verified: `ALERT - Farm Attention Digest` calls the backend summary endpoint, filters empty digests, suppresses repeated content with workflow static data for non-manual executions, and sends only to Charl. Manual executions `49137` and `49138` sent the expected digest for `LIT-2026-8A0F`; manual execution `49136` proved the dry-run/no-output gate. First scheduled duplicate-suppression observation remains pending because manual n8n executions do not reliably persist workflow static data.
