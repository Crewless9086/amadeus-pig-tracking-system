...
- 10.3R irrigation Supabase data model is applied locally/to Supabase and verified locally. It creates empty data tables only: zones, daily plans, plan items, state snapshots, events, auxiliary devices/tasks, and sensor states. Backend deploy is still needed before Render can verify `/health/database/irrigation-schema`. No data import, dashboard, Oom Sakkie control, IFTTT call, or `2.3.2` change has been made.
