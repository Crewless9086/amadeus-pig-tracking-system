...
- Phase 9.7H first correction is local: Add Litter now uses `born_alive` for active/on-farm piglet rows and creates `stillborn_count` rows as `Dead` / `On_Farm = No` with birth-date exit date and `Exit_Reason = Stillborn`, preserving dead pig rows for history. Stillborn and planned pre-weaning death reasons now count as `dead` in lifecycle outcome summaries. Full local unittest suite passed at 337 tests on 2026-06-04. Remaining 9.7H work is the litter detail fast `Mark Piglets Dead` preview/apply action.
