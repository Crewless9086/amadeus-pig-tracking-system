...
- Phase 9.7I smart return navigation is deployed/working with local follow-up fixes: litter piglet table links carry `return_to=/litter/<litter_id>` and `return_label=Back to Litter`, and pig detail uses that safe internal return context to replace the header back link when opened from a litter. Owner deployed and tested this path successfully. Local quick fix changes `/litter/<litter_id>` header link to `Back to Dashboard` instead of Pig List. First return-navigation sweep also covers pig profile child pages: weight capture/history, treatment/history, movement/history, and family tree now return to the pig profile when opened from pig detail. Remaining follow-up is to walk through sales, reports, matings, dashboard-origin, and list/detail return paths.
