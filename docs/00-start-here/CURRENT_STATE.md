...
- Phase 10.7D Oom Sakkie kiosk review advisor panel is local-ready: `/oom-sakkie` now includes a manual `Review Advisor` panel under the review summary. It fetches the advisory endpoint on refresh and shows guard state, suggested actions, and prioritized queue items using text-only DOM rendering. It does not auto-poll, auto-mark feedback, call a model, execute tools, change Telegram, create a specialist loop, or write farm data.
- `docs/00-start-here/CLAUDE_REVIEW_HANDOFF.md` now has a filled current-review prompt for the Oom Sakkie 10.6 through 10.7D batch. Use it for the next Claude review instead of the older generic template.
