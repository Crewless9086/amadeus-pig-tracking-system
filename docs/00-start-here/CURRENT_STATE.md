...
- Phase 9.2B is implemented locally and awaiting deploy/browser verification: `/pigs` now displays numeric-only tags as three digits, sorts the list with numeric-aware tag order, and keeps profile links on unchanged `pig_id` values.
...
- Phase 9.3B is implemented locally and awaiting deploy/browser verification: `/pig-weights` weight input blocks mouse-wheel value changes, hides number spinners, and shows a top save action after the required fields while preserving the existing save payload.
...
- Next step is choosing the next Supabase slice deliberately: feature-flagged read model, broader completed-order import process, or Phase 10.3 telemetry review.
