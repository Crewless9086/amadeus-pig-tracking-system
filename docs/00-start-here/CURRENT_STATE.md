...
- 10.3P read-only irrigation status endpoint is implemented locally. `GET /api/telemetry/irrigation/status` reads the existing irrigation sheet, returns `mode = read_only`, disables all hardware-control flags, and locally read-verified today's plan from `Amadeus_Irrigation_Logs` without writing to Sheets or Supabase.
