...
- Herd dashboard count audit is locally fixed: backend herd counts reconciled to `103` on-farm pigs once `Gilts`, `Piglets`, and `Growers` were included in the visible tile. Dashboard now displays the full counted breakdown locally; deploy/browser verification remains pending.
