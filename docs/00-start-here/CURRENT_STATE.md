...
- Oom Sakkie voice operating agent PRD was added on 2026-06-06 at `docs/01-architecture/OOM_SAKKIE_VOICE_OPERATING_AGENT_PRD.md` and updated after owner clarified the local farm PC/screen goal plus Trillion inspiration. Claude review was logged on 2026-06-06: revise first, avoid split-brain behavior with existing n8n `2.0`, and build a narrow Phase 10.6A read-only kiosk MVP before voice or advanced agents. Current open decision before code: confirm Flask/backend as the single long-term Oom Sakkie brain, with n8n/GateKeeper later forwarding Telegram to `/api/oom-sakkie/message`.
