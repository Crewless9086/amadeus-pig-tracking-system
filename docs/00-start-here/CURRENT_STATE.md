...
- Phase 9.7G smart wean/tag attention timing is local: backend computes estimated wean date as birth/farrowing date + 35 days, starts tag/wean attention 3 days before estimated wean date, suppresses early tag-number dashboard reminders, and exposes timing fields on litter detail. Newborn health attention no longer requires earmark fields, and the newborn-health UI hides the earmark checkbox so first-days treatment is Panacur/Ecomectin only. Real-data check on 2026-06-04 for `LIT-2026-9E4A` returned estimated wean date `2026-06-22`, attention start `2026-06-19`, and no dashboard attention today. Full local unittest suite passed at 336 tests.
