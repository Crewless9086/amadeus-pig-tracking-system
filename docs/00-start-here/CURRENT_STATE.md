...
- Phase 9.1C is implemented locally: litter detail now shows attention metadata and provides a backend-owned `Mark as Weaned` action that updates `LITTERS` and linked active/on-farm `PIG_MASTER` piglets with the chosen wean date/count. Purpose/classification review remains a later slice. A 2026-05-30 audit confirmed `LIT-2026-OTY0` and `LIT-2026-0LBF` were record-reconciliation attention items and `LIT-2026-8A0F` needs piglet tag numbers. Owner reconciled `LIT-2026-OTY0` and `LIT-2026-0LBF`; local dashboard logic now only keeps `Weaned - review purpose` when active/on-farm piglets still have blank or `Unknown` purpose, so older litters with accepted `Sale` / `Grow_Out` / `Breeding` purposes no longer wait on the future auto-classification function. `LIT-2026-8A0F` remains the legitimate visible attention item until its piglets are tagged.
...
- Sales summary screen idea is captured in `NEXT_STEPS.md` Phase 9.5B as a future dashboard/reporting slice: one full overview for all sales streams with date/month/year filters, totals, Rand values, and drill-in paths.
- Supabase Security Advisor warning received on 2026-05-27 for `rls_disabled_in_public`. RLS hardening migration `supabase/migrations/202605270001_enable_rls_on_public_tables.sql` is prepared to enable RLS on all current public tables with no anon/auth policies. Backend-owned `DATABASE_URL` access remains the intended path; browser/Data API access should stay closed by default.
