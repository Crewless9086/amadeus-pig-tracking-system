...
- `docs/00-start-here/CLAUDE_REVIEW_HANDOFF.md` now has a self-contained current-review packet for the Oom Sakkie 10.6 through 10.7O batch. In Claude Code, the owner can say `Read docs/00-start-here/CLAUDE_REVIEW_HANDOFF.md and run the current review`; the file itself lists scope, files, commands, design questions, out-of-scope items, and deliverable format. Do not spend a Claude review on every small router adjustment; batch the next review after several real kiosk traces or a meaningful new feature slice.
