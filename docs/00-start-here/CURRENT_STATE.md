...
- GS-MIG-8 is active on `gs-mig-8-complete-supabase-cutover`: complete remaining order/sales workflow cutover after PR #27.
...
- Remaining Google Sheets dependencies are now narrower: document settings/generation/send writes, mating/breeding mutation routes, legacy setup/import scripts, and some formula-specific farm/litter attention replacement work.
