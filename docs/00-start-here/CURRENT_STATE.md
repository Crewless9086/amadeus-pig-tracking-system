...
- Phase 9.7F litter newborn health attention first backend/UI slice is local: `POST /api/pig-weights/litter/<litter_id>/newborn-health` supports dry-run-first earmark plus antiparasitic/deworming/vaccination capture for all active/on-farm piglets in a litter, using `PRODUCT_REGISTER` for treatment products and `MEDICAL_LOG` for treatment records. Earmark truth uses structured `PIG_MASTER` columns `Earmarked` and `Earmark_Date` at AK/AL. Real dry-run against `LIT-2026-9E4A` with `PRD-001 Ecomectin 1%` and `PRD-002 Panacur 4%` previewed 10 piglet earmark updates and 20 medical rows without writing: 10 `Antiparasitic` rows and 10 `Deworming` rows. `/litter/<litter_id>` now has a product-dropdown preview/apply action panel that appears only when newborn health/earmark/deworm attention is indicated. Backend litter attention now gives newborn health priority before tag-number attention for active, not-yet-weaned litters, while excluding old/weaned litters from live newborn-health reminders so they can be handled later by a deliberate historical backfill/reconciliation tool. The litter detail page has also been widened into a two-column operational layout with summary/piglets in the main column and lifecycle/action panels in the side column. Full local unittest suite passed at 333 tests on 2026-06-02.
...
- New Phase 10 follow-up notes are captured in `NEXT_STEPS.md`: herd tile total/breakdown audit, Telegram digest/reminders for farm attention, alert message formatting polish, weather/solar dashboard symbols, future task/reminder/project management, and possible Windy weather-station integration research.
