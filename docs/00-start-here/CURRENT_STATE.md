...
- Farm attention Telegram digest first backend slice is local: `GET /api/reports/farm-attention-summary` combines existing order attention and litter attention into a read-only digest contract for n8n. It sends no Telegram messages and writes nothing to Supabase or Google Sheets.
