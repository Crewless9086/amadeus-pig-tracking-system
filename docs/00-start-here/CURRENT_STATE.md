...
- Phase 8E breeding board sorting is locally implemented: `/matings` now sorts action sections by operational urgency/date, closed records by newest actual farrowing/fallback date, and keeps closed/linked-litter records out of action sections. Local focused checks and the full local unittest suite passed on 2026-06-01; deploy/browser verification remains before closure.
