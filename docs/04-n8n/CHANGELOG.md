...
**Restart-recovery smoke on 2026-05-17:** Safe conversation `1774` contained leftover draft `ORD-2026-683FC3` from the interrupted test. Header and one active line were correct, but no quote document existed. Production `send-latest` and direct production quote generation returned `500`; local repo code generated `DOC-2026-8D3420` / `Q-2026-683FC3` successfully against live Sheets/Drive, then production sent that existing quote successfully and marked it `Sent`. Cleanup cancelled the order, closed intake `INTAKE-2026-25FCA7`, cleared Chatwoot attributes, and verified active order/intake lookups returned `no_match`. Phase 5.9 still needs a fresh production one-turn pass without local recovery.

**Fresh smoke after cleanup:** Exact one-turn message on safe conversation `1774` created `ORD-2026-644D1A`, one active Female Grower `35_to_39_Kg` line, Cash, Riversdale, and generated `DOC-2026-02ADA4` / `Q-2026-644D1A`, but the quote stayed `Generated`. Direct backend `send-latest` then sent it successfully, proving backend delivery is healthy and the live n8n create path did not run or preserve the post-create send result. The intake also remained unlinked (`INTAKE-2026-56A068`, blank `draft_order_id`), matching a stale/lean `1.2` create result. Repo patch: `1.0 Code - Store Draft Order Context` now recovers conversation/intake/payment metadata from `Set - Draft Order Payload` when `1.2` does not echo it. Local verification passed: workflow JSON parsing, all Code-node JavaScript checks, and `1.2` create-with-lines send branch connection checks. Cleanup cancelled the order, closed the intake, cleared Chatwoot attributes, and verified conversation `1774` returned `no_match`.

**Final slice-2 live verification:** Uploaded active `1.0` (`V73HaIqVpzv44SFc`) and `1.2` (`YDRs6fwde7MzPYn7`) via n8n API. Fixes: `1.2` no longer sends `send_quote_if_ready=true` into backend create-with-lines, post-create quote sending is a separate branch with a 45-second cooldown before `send-latest`, and `1.0 Code - Store Draft Order Context` is back to pass-through because `1.2` echoes the needed metadata. Exact one-turn smoke on safe conversation `1774` passed with `ORD-2026-6E5A81`: draft and active line created, intake `INTAKE-2026-F787C6` linked to the draft, quote `DOC-2026-E8A19A` / `Q-2026-6E5A81` generated and sent through `1.5`, and `ORDER_DOCUMENTS.Document_Status = Sent` with `Sent_By = Sam Phase 5.9 intake`. n8n executions succeeded (`1.2 #44579`, `1.0 #44581/#44582`). Cleanup cancelled the order, closed the intake, cleared Chatwoot attributes, and active lookups returned `no_match`.
