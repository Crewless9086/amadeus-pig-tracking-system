...
def arguments(child=None, parent=None, correction=None):
    ...
    manifest = {"version": adapter.VERSION, "mission_id": adapter.MISSION_ID, "parent_mission_id": adapter.PARENT_ID,
        "candidate": candidate, "generation": "synthetic-new-generation", "idempotency_key": "synthetic-runtime-readiness-rebind",
...
class ReconciliationTests(unittest.TestCase):
    ...
    def test_exact_web_and_scheduler_scope_rejects_broader_effects_and_rollback(self):
        changes = {
            "other_scheduler": ("allowed_effects", f"existing_scheduler_application_release:{adapter.SCHEDULER_SERVICE}",
                                "existing_scheduler_application_release:crn-synthetic-other"),
            "generic_scheduler": ("allowed_effects", f"existing_scheduler_application_release:{adapter.SCHEDULER_SERVICE}", "cron_deploy"),
            "wrong_web_rollback": ("allowed_effects", f"application_revision_rollback:web:{adapter.WEB_ROLLBACK}",
                                   f"application_revision_rollback:web:{adapter.SCHEDULER_ROLLBACK}"),
            "wrong_scheduler_revision": ("allowed_effects", f"application_revision_rollback:scheduler:{adapter.SCHEDULER_SERVICE}:{adapter.SCHEDULER_ROLLBACK}",
                                         f"application_revision_rollback:scheduler:{adapter.SCHEDULER_SERVICE}:{adapter.WEB_ROLLBACK}"),
            "other_scheduler_rollback": ("allowed_effects", f"application_revision_rollback:scheduler:{adapter.SCHEDULER_SERVICE}:{adapter.SCHEDULER_ROLLBACK}",
                                         f"application_revision_rollback:scheduler:crn-synthetic-other:{adapter.SCHEDULER_ROLLBACK}"),
            "missing_other_scheduler_prohibition": ("forbidden_effects", next(iter(adapter.ADDED_FORBIDDEN_EFFECTS)), None),
            "wrong_other_scheduler_prohibition": ("forbidden_effects", next(iter(adapter.ADDED_FORBIDDEN_EFFECTS)),
                                                 "cron_deploy_other_than:crn-synthetic-other"),
        ...
        for effect in ("existing_web_application_release:srv-synthetic-other", "manual_cron_trigger",
                       "application_revision_rollback:web:" + adapter.PREDECESSOR_BASE):
            m,a=json.loads(self.args["manifest_bytes"]),json.loads(self.args["approval_bytes"])
    ...
    def test_exact_service_acceptance_cannot_broaden_or_drop_rollback_bindings(self):
        for before, after in ((adapter.WEB_SERVICE, "srv-synthetic-other"),
                              (adapter.SCHEDULER_SERVICE, "crn-synthetic-other"),
                              (adapter.WEB_ROLLBACK, "0" * 40),
                              (adapter.SCHEDULER_ROLLBACK, "1" * 40)):
            m,a=json.loads(self.args["manifest_bytes"]),json.loads(self.args["approval_bytes"])
    ...
    def test_only_blanket_cron_prohibition_is_replaced_and_other_history_is_preserved(self):
        ...
        self.assertEqual(set(current["forbidden_effects"]),
            (set(prior["forbidden_effects"]) - {"cron_deploy"}) | adapter.ADDED_FORBIDDEN_EFFECTS)
        self.assertNotIn("cron_deploy", current["forbidden_effects"])
    ...
    def test_prior_reconciliation_history_survives_the_next_transition(self):
        ...
        self.assertNotIn("cron_deploy",current["mission_admission_contract"]["forbidden_effects"])
