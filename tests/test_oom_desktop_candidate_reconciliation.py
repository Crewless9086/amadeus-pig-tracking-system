...
def fixtures():
    ...
    prior_contract = {"generation": "synthetic-old-generation", "base_sha": "a" * 40,
        "branch": "synthetic-pr1336", "allowed_files": ["old.py"], "forbidden_files": ["*"],
        "allowed_effects": sorted(adapter.REMOVED_EFFECTS | {"repository_candidate_validation", "merge"}),
        "forbidden_effects": ["cron_deploy", "farm_write", "hardware_command"],
    ...
    prior_receipt = {"status": "valid", "receipt_id": "MAR-" + "A" * 64, "content_sha256": "a" * 64,
        ...
        "generation": prior_contract["generation"], "base_sha": "a" * 40, "head_sha": "b" * 40,
    ...
    metadata = {"mission_family": {"root_mission_id": adapter.PARENT_ID,
        ...
        "review_packet": {"pr_number": 1336, "candidate_revision": "b" * 40, "branch_name": "synthetic-pr1336"},
...
def arguments(child=None, parent=None, correction=None):
    ...
    candidate = {"pr_number": 1343, "branch": adapter.BRANCH, "base_sha": adapter.BASE,
    ...
    manifest = {"version": adapter.VERSION, "mission_id": adapter.MISSION_ID, "parent_mission_id": adapter.PARENT_ID,
        "candidate": candidate, "generation": "synthetic-new-generation", "idempotency_key": "synthetic-rebind-1343",
...
def issued_predecessor(test,connect):
    ...
    prior={"pr_number":1336,"branch":"synthetic-pr1336","base_sha":"a"*40,"head_sha":"b"*40,
        "changed_files":["old.py"],"diff_sha256":canonical_candidate_diff(["old.py"],b"synthetic candidate diff")}
    metadata["review_packet"].update(candidate_diff_sha256=prior["diff_sha256"],changed_files=["old.py"])
...
def issuer_chain(test,connect,snapshot):
    ...
    test.assertEqual(callback(test,connect,old,1336).status_code,409)
    raced=callback(test,connect,old,1336,stale_mission=stale_mission,stale_authority=stale_authority)
...
class ReconciliationTests(unittest.TestCase):
    def setUp(self): self.db = MemoryDB(); self.args = arguments(); self.connect = lambda _: self.db
