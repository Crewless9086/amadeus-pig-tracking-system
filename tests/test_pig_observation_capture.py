...
class PigObservationCaptureTests(unittest.TestCase):
    ...
    def test_observation_persists_factual_event_with_server_author(self):
        ...
        result, status = record_observation(self.observation, connect_factory=lambda _: _Connection(cursor))
        ...
        self.assertEqual(params[3], "authenticated_owner_admin")
    ...
    def test_management_intent_is_advisory_and_never_an_action(self):
        ...
        result, status = record_management_intent(self.intent, connect_factory=lambda _: _Connection(cursor))
        self.assertEqual(status, 201)
    ...
    def test_capture_rejects_invalid_confidence_before_database_write(self):
        result, status = record_observation({**self.observation, "confidence": 1.1}, connect_factory=lambda _: self.fail("must not connect"))
    ...
    def test_capture_fails_closed_when_database_is_not_configured(self):
        with patch.dict("os.environ", {}, clear=True):
            result, status = record_management_intent(self.intent)
    ...
    def test_routes_delegate_distinct_intent_capture(self):
        app = __import__("app").app
        app.config.update(TESTING=True)
        with app.test_client() as client, patch.object(pig_weights_routes, "require_owner_admin_access", return_value=None), patch.object(pig_weights_routes, "record_management_intent", return_value=({"success": True, "advisory_only": True}, 201)) as capture:
            response = client.post("/api/pig-weights/management-intents", json=self.intent)
        self.assertEqual(response.status_code, 201)
        capture.assert_called_once_with(self.intent)
