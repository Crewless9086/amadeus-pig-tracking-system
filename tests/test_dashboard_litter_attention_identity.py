...
def test_dashboard_uses_sow_name_as_primary_litter_attention_identity():
    ...
    assert 'const sowName = first.sow_name || first.sow_tag_number || ""' in source
    assert 'const headline = sowName || first.litter_id || "Litter"' in source
...
def test_dashboard_attention_is_read_only_presentation():
    ...
    assert 'fetch(' not in source[source.index('const sowName = first.sow_name'):source.index('const sowName = first.sow_name') + 700]
