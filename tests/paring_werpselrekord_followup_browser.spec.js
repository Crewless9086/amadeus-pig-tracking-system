...
const linkedMating = {mating_id:"MAT-LINKED",linked_litter_id:"LIT-LINKED",sow_name:"Maya",boar_name:"Bola",sow_current_pen_name:"Kraam Saal 01"};
...
(async () => {
  ...
  assert.equal(await olivePage.locator("#pwr_current_pen").inputValue(), "Kraam Saal 04");
  assert.match(await olivePage.locator("#pwr_current_pen_label").innerText(), /Huidige hok · \d{4}-\d{2}-\d{2}$/i);
