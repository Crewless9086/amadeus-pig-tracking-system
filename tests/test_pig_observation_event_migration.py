...
class PigObservationEventMigrationTests(unittest.TestCase):
    def test_observation_fact_rail_is_additive_append_only_and_owner_safe(self):
        ...
        self.assertIn("revoke all privileges on table public.pig_observation_events from public", sql)
        self.assertIn("revoke all privileges on table public.pig_observation_events from anon", sql)
        self.assertIn("revoke all privileges on table public.pig_observation_events from authenticated", sql)
        self.assertIn("revoke all privileges on table public.pig_observation_events from service_role", sql)
        self.assertIn("grant select, insert on table public.pig_observation_events to service_role", sql)
        self.assertIn("revoke all privileges on function public.pig_observation_events_validate_supersession()", sql)
        self.assertIn("revoke all privileges on function public.pig_observation_events_block_update_delete()", sql)
