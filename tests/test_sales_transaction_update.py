...
class SalesTransactionUpdateTests(unittest.TestCase):
    ...
    def test_update_rejects_already_cancelled_transaction(self):
        cursor = Mock()
        cursor.fetchone.return_value = ("SALE-1", "Slaughter", "Cancelled", "Cancelled", "Old note")
        _connection, psycopg = _mock_psycopg_connection(cursor)
    ...
    def test_update_changes_header_first_item_and_appends_note(self):
        ...
        cursor.fetchone.side_effect = [
            ("SALE-1", "Slaughter", "Confirmed", "Unpaid", "Original note"),
        ...
        self.assertIn("Original note", header_update_params[5])
        self.assertIn("Payment update", header_update_params[5])
        self.assertIn("Amount 1200.00 -> 1500", header_update_params[5])
