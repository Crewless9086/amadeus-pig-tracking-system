...
class CharlieOwnerApprovalInboxTests(unittest.TestCase):
    ...
    def test_lists_sam_meat_learning_events_as_read_only_cards(self):
        connection = _MeatLearningConnection([(
            ...
            "Please send the delivery street address or farm name.",
            {"product_type": "half_carcass"},
            {"product_type": "half_carcass", "cut_set": "Set A", "location": "Riversdale"},
            ["delivery_address"],
            ["delivery_or_location"],
            [],
            ["missed_delivery_intent"],
            "needs_followup",
            "Sam should collect the delivery address before owner review.",
            "whatsapp_status",
        ...
        self.assertEqual(item["source_ref"], "1808")
        self.assertEqual(item["conversion_signal"], "needs_followup")
        self.assertFalse(item["decision_supported"])
        self.assertFalse(item["authority"]["sends_customer_message"])
        self.assertFalse(item["authority"]["creates_order"])
        self.assertIn("evidence_only_learning", item["risk_flags"])
