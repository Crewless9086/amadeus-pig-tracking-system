...
def preview_authoritative(payload, *, channel="application", connect_factory=None):
    action, errors = normalize_action(payload, channel=channel)
    if errors:
    ...
    return preview_action(payload, channel=channel, identity_rows=identities)


def execute_action(payload, *, actor_id, confirmed_preview_digest, channel="application", connect_factory=None):
    ...
    if str(confirmed_preview_digest or "") != preview["preview_digest"]:
