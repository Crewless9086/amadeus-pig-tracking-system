-- Keep a recovered pre-attempt GREEN lease bound to the commissioned device.
-- This migration performs no claim, print, provider call, or farm mutation.
...
begin
  ...
  if v_job.lease_token is distinct from p_lease_token or v_job.lease_owner<>p_worker_id
     ...
     or not exists(select 1 from app_private.document_print_device_registry r
       where r.farm_scope_id=v_job.farm_scope_id and r.green_id=v_job.green_id
         and r.printer_id=v_job.printer_id and r.cups_queue_id=v_job.cups_queue_id
         and r.registry_version=v_job.registry_version and r.active
         and r.commissioned_at is not null) then raise exception 'lease renewal invalid'; end if;
  ...
  if v_job.lease_expires_at>clock_timestamp() or p_lease_seconds not between 30 and 300
     ...
     or not exists(select 1 from app_private.document_print_device_registry r
       where r.farm_scope_id=v_job.farm_scope_id and r.green_id=v_job.green_id
         and r.printer_id=v_job.printer_id and r.cups_queue_id=v_job.cups_queue_id
         and r.registry_version=v_job.registry_version and r.active
         and r.commissioned_at is not null) then raise exception 'lease recovery invalid'; end if;
...
revoke all on function app_private.renew_document_print_job_lease(text,text,text,integer,text,text,text,text,text)
  from public,anon,authenticated;
revoke all on function app_private.recover_document_print_job_lease(text,text,integer,text,text,text,text,text)
  from public,anon,authenticated;
grant execute on function app_private.renew_document_print_job_lease(text,text,text,integer,text,text,text,text,text),
