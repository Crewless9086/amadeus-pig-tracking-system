...
Version 0.3.7 uses the Home Assistant supported prebuilt image reference
...
Publish no inbound ports or tunnels. Configure `printer_certificate_sha256` with the independently verified lowercase 64-character fingerprint of the private IPPS printer certificate. Green retrieves a certificate only from `printer_endpoint_ip`, rejects any fingerprint mismatch, writes the matched public certificate into its ephemeral runtime, then performs normal hostname/TLS verification against the `printer_uri` hostname. Green maps no Home Assistant or app configuration directory for trust. Certificate rotation requires a newly verified fingerprint before restart. Choose exactly one canonical transport profile. `private_pinned` preserves the private-network design. Commission a non-empty `canonical_endpoint_ip` from the complete private DNS answer set; HTTPS connects only to that address while TLS verifies the configured hostname. For `public_pki_exact_origin`, leave `canonical_endpoint_ip` blank. Home Assistant saves that explicit blank and the worker normalizes it to no runtime pin. This public profile accepts only `https://amadeus-pig-tracking-system.onrender.com`, uses the system public trust store with hostname verification, follows no redirects, and forbids a non-empty endpoint pin, alternate port, path, credentials, query, or fragment. Canonical claim, command, transition, and PDF paths remain source-fixed; every request retains bearer, farm, and Green identity bindings.
