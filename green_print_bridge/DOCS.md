...
The printer transport is always `private_ipps`; plain IPP and disabled TLS are unsupported. Set `printer_endpoint_ip` to the commissioned private address. An IP-literal `printer_uri` requires that same address and a certificate with the IP SAN. A hostname `printer_uri` requires a certificate with that hostname SAN and complete DNS resolution to exactly one private address equal to `printer_endpoint_ip`; public, empty, ambiguous, or drifted answers hold before CUPS starts. Enter values only in the Home Assistant app options UI.
