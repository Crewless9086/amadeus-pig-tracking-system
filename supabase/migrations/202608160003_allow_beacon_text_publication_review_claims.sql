...
begin
  ...
  alter table app_private.oom_protected_action_claims
    ...
    check(action_kind in ('mortality','grouped_weights','herdmaster_breeding_grouped',
      ...
      'rootline_fertilizer_mixer_presence_refresh','beacon_text_only_publication_review'));
end $$;

revoke all privileges on table app_private.oom_protected_action_claims
  from public, anon, authenticated;

insert into app_private.migration_log(migration_id, description)
values('202608160003_allow_beacon_text_publication_review_claims',
       'Allow protected BEACON text-only publication review claims')
