-- Complete the Herdmaster evidence and advisory planning rails.
--
-- Additive and unapplied. Observation events remain factual evidence only.
-- Management intents are non-executing advisory records only. Neither rail
-- changes pig purpose, lifecycle, current state, orders, sales, reservations,
-- slaughter, customer messages, notifications, or any operational projection.

-- The preceding unapplied observation migration used verbose factual names.
-- Align that rail to the evidence contract without changing row values.
