...
-- Align the initial factual-observation names with the protected capture
-- contract. This changes neither row values nor the pig current-state projection.
...
-- There is deliberately no anonymous or authenticated-browser write policy.
-- A future protected backend capture rail must derive its actor server-side.
...
create policy pig_observation_events_service_role_insert
    on public.pig_observation_events for insert to service_role with check (true);
...
create policy pig_management_intent_events_service_role_insert
    on public.pig_management_intent_events for insert to service_role with check (true);
...
begin
    if new.observation_event_id is not null and not exists (
        select 1 from public.pig_observation_events observation_event
    ...
    if new.supersedes_management_intent_event_id is not null and not exists (
        select 1 from public.pig_management_intent_events prior_event
